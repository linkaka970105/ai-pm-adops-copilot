...
| 核心能力 | 意图识别、RAG、数据查询、指标拆解、历史案例检索、客户回复生成、PromptOps、工具网关、人工确认闭环 |
...
|---|---|---|---|
| 总控模型 | 意图分类、风险识别、路由 | 轻量 LLM 或小型分类模型 | 每次请求必经，需低延迟、低成本、稳定输出 |
| 诊断生成模型 | 复杂排障推理、摘要、客户回复 | 支持工具调用和长上下文的高能力 LLM | 需要理解广告语义、跨数据源综合、生成结构化输出 |
| Embedding 模型 | 文档向量化 | 中文和英文广告术语表现稳定的 embedding 模型 | 广告文档常混合中英文、缩写和平台术语 |
| 重排序模型 | 提升召回准确性 | Cross-encoder 或文本重排序模型 | 仅靠向量召回容易命中语义相近但场景错误的文档 |
...
| 能力 | 选型 | 理由 |
|---|---|---|
| 意图识别 | 轻量 LLM + few-shot 分类 Prompt，后续可用标注数据训练小模型 | V1 需求变化快，Prompt 迭代成本低；稳定后再训练分类模型降成本 |
| 风险识别 | 规则 + 轻量模型双层判断 | 高风险动作有明确关键词和动作类型，规则更可控；模型补充语义判断 |
| 路由 | 状态机 | 可审计、可回放、便于测试，不依赖自由 Agent 自行决定 |
| 回复审核 | 规则拦截 + LLM 审核 | 越权、高风险动作可规则拦截；话术风险可用模型辅助判断 |
...
| 原因排序 | 规则得分 + LLM 生成解释 | 原因排序需结合数据、SOP 和历史案例；LLM 负责组织语言 |
...
| 原因解释 | RAG + LLM | SKAN、SAN、Postback 等概念需要引用文档并转成客户可理解语言 |
| 客户话术 | 模板 + LLM 改写 | 对外回复必须稳妥、口径一致 |
...
| 能力 | 选型 | 理由 |
|---|---|---|
| 文案识别 | 文本规则 + LLM 分类 | 夸大、承诺、敏感词可规则识别，语义风险需模型判断 |
| 图片/视频理解 | V1 支持人工粘贴描述，V1.1 接入多模态识别 | V1 先降低实现复杂度，避免多模态误判带来审核风险 |
| 政策检索 | 平台/国家/行业元数据过滤 + RAG | 审核政策高度依赖平台、行业和国家 |
...
| 能力 | 选型 | 理由 |
|---|---|---|
| 错误码解释 | 结构化错误码表 + RAG | 错误码需要精确匹配，文档解释需要引用 |
| 日志查询 | 受控日志模板 | 日志敏感且字段复杂，必须白名单查询 |
| 链路判断 | 规则链路图 | request、fill、show、click、callback 有明确链路顺序 |
| 升级信息包 | 模板生成 | 技术支持需要稳定字段，减少反复补信息 |
...
3. 元数据先行：每个知识片段必须带来源、版本、平台、场景、权限、风险等级。
...
V1.0 选择中文和英文混合语义表现稳定的 embedding 模型。广告文档中存在 Campaign、Creative、Placement、Postback、SKAN、SAN、CPI、CPA、ROAS 等大量英文缩写，模型必须能处理中英文混合语义。
...
↓
向量召回：语义相似 SOP、历史案例、政策说明
↓
结构化查询：指标字典、归因口径、错误码表
↓
重排序：按场景相关性、来源权威性、版本新鲜度排序
↓
Top-K 引用压缩
↓
传入子智能体生成答案
...
| 检索策略 A/B | 对比 Top-K、Rerank、元数据过滤策略 | 引用支撑率、召回准确率 |
