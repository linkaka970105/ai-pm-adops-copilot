...
| 3. 意图与实体识别 | 原始 query、页面上下文 | 抽取 MMP、metric、event、时间窗、差异方向；判断是否属于归因核对 | `intent=attribution_discrepancy_check`，`metric=installs`，`reported_gap=28%`，`time_range=yesterday` | 缺少 event 时默认使用 `install`，但诊断卡标记为默认假设 |
...
| RAG Query 改写 | `workflow_executor` 中的检索前处理 | 是 | 把原始 query 改写成中英文语义 query、实体 query 和场景核查 query | 权限过滤、召回、rerank、引用选择、最终回答 | `6.6.4.1` |
...
- postback_status: 聚合 postback 成功率、失败、拒收、延迟和重试摘要，不包含 raw URL 或设备级日志。
