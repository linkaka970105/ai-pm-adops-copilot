...
| 人工采纳率 | 点击采纳、复制内部摘要、基于建议完成下一步检查或升级工单的会话数 | Copilot 已输出诊断的会话数 | 前端事件、用户反馈、抽检 | 灰度期达到 60% 以上 |
...
| --- | --- | --- | --- |
| `有效会话数_by_issue_type` | Copilot 会话完成后，用户点击采纳、复制内部摘要、基于建议完成下一步检查或升级工单时计入 | `session_created`、`intent_routed`、`diagnosis_generated`、`diagnosis_accepted`、`summary_copied`、`suggested_action_completed`、`ticket_escalated` | 剔除测试账号、无权限会话、范围外问题、无证据输出和重复刷新 |
...
问题输入
  ...
  -> 总控生成 RAG Query 改写计划，场景智能体按检索计划调用 RAG、报表工具、归因工具获取上下文
...
以下案例用于说明系统不是只做“问答”，而是完成从用户提问、权限路由、工具查询、证据对象生成、场景 workflow 输出、安全交付到人工确认的完整闭环。
...
| 5. 证据对象生成 | 生成差异证据、口径证据、postback 证据、待补充信息等 evidence object | evidence_id、来源、时间、Owner、对象级置信度 | 没有证据的假设不得进入主结论，只能进入“待确认” |
| 6. 场景 workflow 输出 | 输出 `workflow_output`，包含可能原因排序、已排除项、下一步动作和内部解释摘要 | `attribution_discrepancy_v1` 结构化结果 | 不生成最终前端展示对象，也不生成可直接发送客户的回复 |
...
```json
{
  ...
  "page_context": {
    ...
    "geo": "US",
    "timezone": "UTC",
    "time_range": "2025-01-20/2025-01-20"
  }
}
```
...
| 3. 意图与实体识别 | 原始 query、页面上下文 | 抽取 MMP、metric、event、时间窗、差异方向；判断是否属于归因核对 | `intent=attribution_discrepancy_check`，`metric=installs`，`event_name=install`，`reported_gap=28%`，`time_range=yesterday` | 当 metric 明确为 installs/install discrepancy 时，缺少 event 可默认 `event_name=install`，但诊断卡标记为默认假设；其他事件不得默认 |
...
| 7. 报表工具查询 | campaign、event、time_range、timezone | 调用平台报表和 MMP 报表，规则层计算差异比例 | 平台 installs=1250，MMP installs=900，差异率=28.0% | API 超时则读取有效缓存并标注 `data_freshness` |
| 8. Postback 摘要查询 | app_id、event、time_range | 查询 postback 聚合状态，不展示原始 token 或用户级日志 | `success_rate=92.4%`，`delayed_rate=6.8%`，无大面积 reject | 无权限查明细时只返回聚合摘要 |
| 9. 证据对象生成 | RAG 引用、工具结果、规则计算结果 | 将不同来源转成统一 evidence object，并绑定 claim | 生成 4 个 evidence object：差异证据、口径证据、postback 证据、待确认项 | 无 evidence 不允许进入主结论 |
| 10. 诊断生成 | evidence object、业务规则、Prompt 模板 | 场景智能体生成 `workflow_output`，包含原因排序、已排除项、下一步动作和置信度计算输入；`confidence_final` 由 workflow 规则层回写 | 主判断：差异真实存在；优先检查 timezone/window/event mapping，postback 延迟为次要可能 | 有冲突证据时输出“需要人工确认”，不做强结论 |
| 11. 安全交付 | `routing_final`、`workflow_output`、`evidence_objects`、权限范围 | 总控检查无证据强答、越权、客户可见话术和敏感信息，输出 `delivery_guard_output` | 最终前端展示内部诊断卡时使用 `delivery_guard_v1` schema；客户沟通需 AM 人工改写确认 | 发现敏感字段时删除并记录安全拦截 |
| 12. 人工反馈 | 采纳/无效/继续追问/升级 | 写入反馈事件，必要时生成 Badcase 或升级工单摘要 | `feedback=accepted_with_followup`，生成内部摘要和工单上下文 | 无反馈会话不计入“有效 AI 辅助排障会话数” |

### 5.2.3 核心中间结果样例

`routing_final`：总控路由候选经过 Schema Guard 和规则层复算后的结果，用于选择固定 workflow，不是最终前端展示对象。
...
```json
{
  ...
  "checklist": [
    {"check": "timezone", "required_evidence": ["platform_timezone", "mmp_timezone"]},
    ...
    {"check": "dedup_or_reattribution", "required_evidence": ["dedup_policy", "reattribution_policy"]},
    {"check": "privacy_or_invalid_traffic", "required_evidence": ["privacy_filtering", "invalid_traffic_filtering"]},
    {"check": "channel_mapping", "required_evidence": ["campaign_id_mapping", "publisher_mapping"]},
    {"check": "data_freshness", "required_evidence": ["last_sync_time", "etl_batch_time"]}
  ],
...
证据对象、场景 workflow 输出和最终安全交付输出：

以下三类 JSON 分属不同环节，不能合并成一个 `diagnosis` 对象。`evidence_objects` 是 Evidence Store 中的证据源对象；`workflow_output` 是归因核对 workflow 的结构化输出；`delivery_guard_output` 才是最终给前端渲染内部诊断卡的结构化结果。`delivery_guard_output.confidence_final` 由 workflow/规则层根据 `confidence_rule_inputs` 回写，不由场景模型或安全交付模型自由打分。

`evidence_objects`：Evidence Store 中可被引用的证据对象。
  ...
  "evidence_objects": [
    {
      ...
      "source_id": "tool_get_platform_report_get_mmp_report_001",
      ...
      "owner": "Ad Reporting",
      "data_freshness": "2025-01-21T01:30:00Z",
      "retrieved_at": "2025-01-21T01:35:00Z"
    },
    {
      "evidence_id": "ev_policy_001",
      "source_type": "knowledge_chunk",
      "source_id": "doc_af_policy_202501",
    ...
    },
    {
      ...
      "source_id": "tool_get_postback_summary_001",
      "claim_supported": "Postback success rate is 92.4%, delayed rate is 6.8%, and reject rate is 0.8% for cmp_A on 2025-01-20.",
      "confidence": 0.88,
      "visibility": "internal",
      "customer_visible_allowed": false,
      "owner": "Postback Platform",
      "data_freshness": "2025-01-21T01:35:00Z",
      "retrieved_at": "2025-01-21T01:35:00Z"
    }
  ]
}
...
`workflow_output`：`wf_attribution_discrepancy_v1` 的场景结构化输出，不作为最终前端展示对象。
...
```json
{
  "workflow_output": {
    ...
    "trace_id": "tr_20250121_0001",
    "difference_rate": "28.0%",
    "checked_items": [
      {"item": "timezone", "status": "needs_followup", "evidence_id": "ev_policy_001", "reason": "Timezone alignment must be confirmed before comparing platform and MMP reports."},
      {"item": "attribution_window", "status": "needs_followup", "evidence_id": "ev_policy_001", "reason": "MMP and platform attribution windows are not both confirmed."},
      {"item": "event_mapping", "status": "needs_followup", "evidence_id": null, "reason": "Platform install event and MMP install event mapping still need confirmation."},
      {"item": "postback_delay", "status": "not_supported_by_evidence", "evidence_id": "ev_postback_001", "reason": "Postback summary does not show a large-scale failure."},
      ...
      {"item": "data_freshness", "status": "needs_followup", "evidence_id": "ev_metric_001", "reason": "Platform and MMP report freshness should be checked before external communication."}
    ],
    "likely_reasons": [
      {"reason": "timezone_or_attribution_window_mismatch", "rank": 1, "evidence_ids": ["ev_metric_001", "ev_policy_001"]},
      {"reason": "mmp_refresh_lag", "rank": 2, "evidence_ids": ["ev_metric_001"]}
    ],
    "excluded_reasons": [
      {"reason": "large_scale_postback_failure", "evidence_ids": ["ev_postback_001"], "why_excluded": "reject_rate=0.8% and delayed_rate=6.8% do not indicate a large-scale postback outage."}
    ],
    "required_followups": ["confirm MMP timezone setting", "compare install event mapping", "rerun report after next MMP sync"],
    "confidence_rule_inputs": {
      ...
      "primary_reason_evidence_count": 2,
      "has_core_data_conflict": false,
      "freshness_unknown": true
    },
    "internal_explanation_summary": "The 28.0% gap is confirmed. The strongest current hypothesis is timezone/window mismatch; postback health does not show a large-scale failure.",
    "requires_human_review": true
  }
}
```

`delivery_guard_output`：`delivery_guard_v1` 的最终交付输出，前端内部诊断卡按此 schema 渲染。

```json
{
  "delivery_guard_output": {
    "schema_version": "delivery_guard_v1",
    "trace_id": "tr_20250121_0001",
    "delivery_type": "internal_diagnosis",
    "risk_level_final": "medium",
    "confidence_final": 0.78,
    "safe_summary": "The 28.0% gap is confirmed. The strongest current hypothesis is timezone/window mismatch; postback health does not show a large-scale failure.",
    "evidence_refs": [
      {"evidence_id": "ev_metric_001", "usage": "supports_summary"},
      {"evidence_id": "ev_policy_001", "usage": "supports_summary"},
      {"evidence_id": "ev_postback_001", "usage": "supports_limitation"}
    ],
    "redactions_applied": [],
    "blocked_reasons": [],
    "limitations": [
      "MMP timezone, attribution window, event mapping, channel mapping and data freshness still require human confirmation before external communication.",
      "This is an internal diagnostic card and is not a customer-visible reply."
    ],
    "next_actions": ["confirm MMP timezone setting", "compare install event mapping", "rerun report after next MMP sync"],
    "customer_visible_reply_allowed": false,
    "requires_human_review": true
  }
}
```
...
| 数据类型 | 示例 | 用途 | 风险控制 |
| --- | --- | --- | --- |
| 广告报表数据 | spend、impressions、clicks、installs、cvr、cpa、roi | 投放异常诊断 | 只读查询、账户权限过滤、聚合展示 |
...
| 向量模型 | BAAI bge-m3 或 Alibaba text-embedding-v3；OpenAI text-embedding-3-large 仅作为合规可用时的离线对照 | 模型切换必须重跑召回评测，不允许只替换模型名 |
| 向量维度 | bge-m3 默认 1024；text-embedding-v3 / text-embedding-3-large 按供应商模型版本记录 | 向量库 schema 需要按模型版本记录 dimension |
...
```json
{
  "retrieval_plan": {
    ...
    "must_have_entities": ["mmp", "event_name", "time_range", "campaign_id"],
    "semantic_queries": [
...
#### 6.6.4.1 Query 改写 Prompt 设计

Query 改写是检索前处理，不生成业务答案，也不选择最终引用。它的目标是把用户自然语言、总控路由结果和业务实体转成可检索的 query 计划，提高中英文混合知识库的召回稳定性。

| 变量 | 填入内容 | 主要用途 |
| --- | --- | --- |
| `{{trace_context_json}}` | trace_id、prompt_version、model、入口、时间 | 关联检索计划、召回结果和回归评测 |
| `{{routing_result_json}}` | 总控归一后的 intent、entities、risk_level_final、selected_workflow | 限制 query 改写场景和实体范围 |
| `{{user_query_json}}` | 用户原始问题 | 保留用户真实表达和语言 |
| `{{conversation_context_json}}` | 最近多轮已确认实体和澄清信息 | 处理省略指代，不继承未确认实体 |
| `{{knowledge_scope_json}}` | 允许检索的 source_type、业务线、语言、敏感级别 | 限制召回范围，避免跨域检索 |
| `{{entity_dictionary_json}}` | MMP、指标、事件、渠道、地区等标准词表 | 统一 AppsFlyer/AF、install/安装等同义词 |
| `{{retrieval_policy_json}}` | query 数量上限、语言策略、禁止项、权限过滤要求 | 控制成本和安全边界 |

变量来源、获取过程与示例值：

| 变量 | 来源系统 / 生成方 | 获取过程 | 缺失或失败处理 | 示例值 |
| --- | --- | --- | --- | --- |
| `trace_context_json` | Prompt Runner / Trace 服务 | 检索前生成或继承当前 trace，写入 query rewrite prompt 版本 | 缺失时不调用模型，直接用原始 query 检索并记录降级 | `{"trace_id":"tr_rewrite_001","prompt_version":"rag_query_rewrite_v1.0"}` |
| `routing_result_json` | 总控 workflow | 读取规则归一后的 `routing_final`，包含最终 intent 和实体 | 缺失时只做通用知识 query，不生成账户实体过滤 | `{"intent":"attribution_discrepancy_check","entities":{"mmp":"AppsFlyer","event_name":"install"}}` |
| `user_query_json` | Copilot Chat / 工单插件 | 继承原始 query，不使用模型摘要替代 | 空 query 时不改写，返回澄清 | `"AppsFlyer installs are lower than our platform yesterday"` |
| `conversation_context_json` | Session Store | 只读取已确认 account、campaign、app、MMP、time_range | 摘要失败则传空数组 | `[{"summary":"User confirmed campaign C123 and AppsFlyer."}]` |
| `knowledge_scope_json` | Product Config + 权限服务 | 根据用户角色、intent、PRD 阶段和知识权限生成 | 缺失时仅允许 SOP/FAQ，敏感级别不超过 internal | `{"source_type":["attribution_policy","sop"],"locale":["en","zh"],"sensitivity_level_lte":"internal"}` |
| `entity_dictionary_json` | 术语表 / 知识运营后台 | 读取已审核同义词、指标枚举、MMP 名称和事件名 | 缺失时不做同义词扩展 | `{"mmp_alias":{"AF":"AppsFlyer"},"metric_alias":{"安装":"install"}}` |
| `retrieval_policy_json` | RAG 配置中心 | 读取 query 数量、topK、禁止输出项和权限过滤策略 | 缺失时使用保守默认值：最多 3 条 query，不生成历史案例 query | `{"max_queries":4,"require_permission_filter":true,"allow_case_query":true}` |
...
━━━━━━━━
## 需求
：输入 下方「运行时输入」JSON，包含路由结果、用户问题、上下文、知识范围、术语表和检索策略
：输出 一个严格 JSON object，用于 RAG 检索；不得输出 Markdown、解释文字或业务答案

## 运行时输入
以下 JSON 由系统在每次调用前填入。你必须只基于本段变量生成检索计划，不得补造 account、campaign、MMP、event 或权限范围。
...
```json
{
  ...
  "entity_dictionary": {{entity_dictionary_json}},
  "retrieval_policy": {{retrieval_policy_json}}
}
```

## 输入上下文
- trace_context: 当前 trace、prompt 版本、模型和调用时间，用于检索回放与成本归因。
- routing_result: 总控规则归一后的 intent、entities、risk_level_final 和 selected_workflow。
- user_query: 用户原始问题，用于保留真实语言和关注点。
- conversation_context: 最近对话中已经确认的实体和澄清信息。
- knowledge_scope: 当前允许检索的文档类型、语言、业务线和敏感级别。
- entity_dictionary: 已审核术语、同义词、MMP 名称、指标和事件枚举。
- retrieval_policy: query 数量、语言策略、权限过滤和禁止项。

## 角色
你是 AdOps Copilot 的 RAG Query 改写器。你只负责生成检索计划，不负责回答问题、不选择最终引用、不判断业务结论。

## 改写规则
① 至少保留 1 条英文 semantic query；当 user_query 为中文或 mixed 时，补充中文 semantic query。
② attribution_discrepancy_check 必须尽量覆盖 MMP、event/metric、timezone、attribution window、postback、dedup、freshness。
③ campaign_performance_diagnosis 必须覆盖 metric、funnel layer、campaign status、budget/bid、conversion tracking。
④ knowledge_lookup 只生成概念、口径、SOP、FAQ query，不生成账户数据 query。
⑤ entity_filters 只能来自 routing_result、conversation_context、knowledge_scope 和 entity_dictionary，不得猜测。
⑥ 若 retrieval_policy.require_permission_filter=true，必须输出 permission_filter_required=true。

## 禁止
- 不得输出业务答案、诊断结论或客户可见回复。
- 不得生成 raw log、token、secret、设备级数据或用户级数据相关 query。
- 不得扩大到 knowledge_scope 未允许的 source_type 或敏感级别。
- 不得从 few-shot 或常识中补齐当前不存在的 campaign、account、app、MMP。

## 输出 schema
{
  "schema_version": "rag_query_rewrite_v1",
  "trace_id": "string|null",
  "scenario": "campaign_performance_diagnosis|attribution_discrepancy_check|knowledge_lookup|unknown",
  "semantic_queries": [
    {"language": "en|zh", "query": "string", "purpose": "policy|sop|metric_definition|case_recall|faq"}
  ],
  "entity_queries": [
    {"query": "string", "entities_used": ["mmp|metric|event_name|campaign_id|geo|source_type"]}
  ],
  "checklist_queries": ["string"],
  "entity_filters": {
    "source_type": ["string"],
    "mmp": "string|null",
    ...
    "reviewed_status": ["reviewed"],
    "sensitivity_level_lte": "public|internal|restricted"
  },
  "permission_filter_required": true,
  "rewrite_limitations": ["string"]
}
````
...
| customer_visible_allowed | 默认 false，只有 public/external_confirmed 且人工审核后才可 true |
| owner | 继承文档 owner |
| retrieved_at | 检索时间 |
...
| 7 | `workflow_executor` | workflow ID、工具约束、证据需求 | 执行知识检索、只读工具查询、证据对象生成或追问/拒绝 | `workflow_output` | 工具失败时按实时 API > 缓存 > ETL > 人工上传 > 待补充降级 |
| 8 | `delivery_guard` | `routing_final`、`workflow_output`、证据对象、安全规则 | 检查无证据强答、客户可见话术、越权字段、敏感信息 | `delivery_guard_output`，最终前端展示结构 | 高风险内容隐藏并转人工确认 |
| 9 | `trace_feedback` | 输出、模型/Prompt/工具/知识版本、用户反馈 | 写入日志、成本、延迟、Badcase 和评测样本 | 可复盘 trace | 无反馈不计入有效会话 |

#### 7.1.4.2 总控 workflow 大模型调用清单

总控 workflow 中不是每一步都调用大模型。权限、Schema 校验、风险终判、置信度终判、工具调用、差异计算和日志写入都由确定性系统完成；大模型只用于自然语言理解、证据解释、引用回答、安全表达整理和离线评测。所有在线 LLM 调用都必须有独立 `prompt_id`、变量 schema、输出 schema、模型路由和降级策略。

| LLM 调用点 | 所属步骤 | 是否在线链路 | 大模型负责什么 | 不交给大模型的内容 | 对应 Prompt 章节 |
| --- | --- | --- | --- | --- | --- |
| 总控路由候选生成 | `llm_route_candidate` | 是 | 意图识别、实体抽取、风险信号候选、置信度组件、候选工具约束 | 权限终判、风险终判、最终 workflow 分流、工具调用 | `7.1.6.5`、`7.1.6.6` |
| RAG Query 改写 | 总控路由后的检索计划生成 | 是 | 把原始 query 改写成中英文语义 query、实体 query 和场景核查 query | 权限过滤、召回、rerank、引用选择、最终回答 | `6.6.4.1` |
| 投放诊断解释 | `workflow_executor` 进入 `wf_campaign_performance_v1` | 是 | 基于 evidence object 解释异常、排序原因、生成内部 next actions | 指标计算、异常阈值、工具查询、置信度终判、操作变更 | `7.2.6` |
| 归因差异解释 | `workflow_executor` 进入 `wf_attribution_discrepancy_v1` | 是 | 基于固定核查结果解释时区、窗口、事件、postback 等差异原因 | 必查项生成、差异率计算、工具查询、置信度终判 | `7.3.6` |
| 纯知识引用回答 | `workflow_executor` 进入 `wf_knowledge_lookup_v1` | 是 | 基于已选择引用生成知识回答、限制说明和下一步建议 | 无引用强答、账户数据查询、引用筛选、引用置信度终判 | `7.4.6` |
| 总控安全交付整理 | `delivery_guard` | 是，按需调用 | 将 workflow 输出整理成内部可交付结果，删除敏感表达，生成安全提示 | 是否越权、是否拒绝、是否转人工、是否允许客户可见 | `7.1.6.7` |
| Judge AI 离线评估 | `trace_feedback` 后的评测后台 | 否 | 评估证据对齐、完整性、安全、可执行性和 Badcase 类型 | 线上逐次拦截、最终业务结论、自动修复上线 | `7.5.5` |

非 LLM 步骤说明：

| 步骤 | 不调用大模型的原因 |
| --- | --- |
| `session_init` | 只负责 trace、登录态、页面上下文继承，要求稳定可审计 |
| `pre_guard` | 涉及权限、敏感字段、raw data 拦截，必须由规则和权限系统做硬判断 |
| `schema_guard` | JSON、枚举、字段类型和工具白名单校验适合确定性代码 |
| `rule_normalizer` | `risk_level_final`、`confidence_final`、`next_action_final` 必须可复算 |
| `workflow_dispatcher` | 固定 workflow 分流应可测试、可回滚、低成本 |
| 工具查询与 Evidence Builder | 数据读取、差异率计算和证据对象组装必须可追溯 |

#### 7.1.4.3 Intent 到 workflow 的路由表
...
指标、事件和字段枚举在系统内统一使用小写 snake/lowercase，例如 `cvr`、`cpa`、`roi`、`impressions`、`installs`；前端展示可转为 `CVR`、`CPA`、`ROI` 等业务可读写法。
...
## 缺失字段规则
- attribution_discrepancy_check 缺少 account_id、campaign_id/app_id、mmp、time_range 时，应输出 missing_fields，并优先 ask_clarification；当 metric 明确为 installs/install discrepancy 时，可默认 `event_name=install`，其他 event 缺失不得默认。
...
#### 7.1.6.7 总控安全交付 Prompt 设计

本 Prompt 对应 `delivery_guard` 中的“安全交付整理”环节。规则层先完成越权、敏感字段、客户可见、无证据强答和人工确认判断；模型只负责把已允许交付的 workflow 输出整理成内部可读结果，或在规则层要求拒绝/转人工时生成安全、克制的内部提示。

| 变量 | 填入内容 | 主要用途 |
| --- | --- | --- |
| `{{trace_context_json}}` | trace_id、prompt_version、model、入口、时间 | 关联交付版本、日志和回滚 |
| `{{routing_final_json}}` | 规则层归一后的 intent、risk_level_final、confidence_final、selected_workflow | 确认交付场景和最终风险 |
| `{{workflow_output_json}}` | 场景 workflow 输出的结构化结果 | 生成内部诊断卡、知识答案、澄清或拒绝提示 |
| `{{evidence_objects_json}}` | 当前允许展示的证据对象列表 | 保证交付内容绑定 evidence_id |
| `{{guard_decision_json}}` | 规则层安全检查结果：allow、redact、block、human_review | 模型必须服从，不得自行放行 |
| `{{permission_scope_json}}` | 用户可见账户、campaign、app、MMP、字段权限 | 防止输出越权字段 |
| `{{delivery_policy_json}}` | 当前阶段交付策略、禁止表达、语言风格和客户可见限制 | 控制输出边界 |
| `{{language_preference_json}}` | 用户语言、系统语言、query 语言 | 决定内部提示语言 |

变量来源、获取过程与示例值：

| 变量 | 来源系统 / 生成方 | 获取过程 | 缺失或失败处理 | 示例值 |
| --- | --- | --- | --- | --- |
| `trace_context_json` | Prompt Runner / Trace 服务 | 继承当前 trace，写入 `delivery_guard_v1.0` 和模型版本 | 缺失时不调用模型，直接返回规则层安全提示 | `{"trace_id":"tr_attr_001","prompt_version":"delivery_guard_v1.0"}` |
| `routing_final_json` | `rule_normalizer` | 读取规则复算后的最终路由，不读取未经校验的模型路由 | 缺失时只允许输出系统异常或转人工 | `{"intent":"attribution_discrepancy_check","risk_level_final":"medium","confidence_final":0.68}` |
| `workflow_output_json` | `workflow_executor` | 读取投放诊断、归因核对、知识查询、澄清或拒绝 workflow 输出 | 缺失时输出“暂无可交付结果”，不得补造结论 | `{"schema_version":"attribution_discrepancy_v1","likely_reasons":[...]}` |
| `evidence_objects_json` | Evidence Store | 只读取当前用户有权查看且已脱敏的 evidence object | 无证据时只能交付待确认或转人工提示 | `[{"evidence_id":"EVT-201","visibility":"internal","fact":"Timezone mismatch exists."}]` |
| `guard_decision_json` | Safety Rule Engine | 规则层检查敏感字段、客户可见、越权、无证据强答后生成 | 缺失时默认 block 并转人工 | `{"decision":"allow_with_redaction","redact_fields":["raw_postback_url"],"requires_human_review":true}` |
| `permission_scope_json` | RBAC/ABAC 权限服务 | 继承当前用户和对象权限范围 | 权限未知时默认不展示账户/MMP 数据摘要 | `{"accounts":["A001"],"campaigns":["C123"],"fields":["aggregate_metrics"]}` |
| `delivery_policy_json` | Product Config + 安全策略 | 读取当前 PRD 阶段、禁止表达、客户可见限制和内部摘要规则 | 缺失时使用最保守策略：不生成客户可见文本 | `{"allow_customer_visible_reply":false,"forbid_commitment":true}` |
| `language_preference_json` | 用户配置 + query 语言识别 | 结合用户偏好、界面语言和 query 主语言 | 缺失时跟随 query 主语言 | `{"preferred":"zh","query_language":"en","ui_language":"zh"}` |
...
━━━━━━━━
## 需求
：输入 下方「运行时输入」JSON，包含最终路由、workflow 输出、证据对象、规则层安全决策、权限范围和交付策略
：输出 一个严格 JSON object，用于内部交付卡片；不得输出 Markdown、解释文字或客户可直接发送回复

## 运行时输入
以下 JSON 由系统在每次调用前填入。你必须服从 `guard_decision`，不得自行放行、解除脱敏、生成客户承诺或补造缺失证据。

```json
{
  "trace_context": {{trace_context_json}},
  "routing_final": {{routing_final_json}},
  "workflow_output": {{workflow_output_json}},
  "evidence_objects": {{evidence_objects_json}},
  "guard_decision": {{guard_decision_json}},
  "permission_scope": {{permission_scope_json}},
...
## 输入上下文
- trace_context: 当前 trace、prompt 版本、模型和调用时间，用于交付回放与成本归因。
- routing_final: 规则层最终 intent、risk_level_final、confidence_final、selected_workflow 和 next_action。
- workflow_output: 已完成 workflow 的结构化结果，包括诊断、知识回答、澄清、拒绝或人工确认结果。
- evidence_objects: 当前用户有权查看且已脱敏的证据对象。
- guard_decision: 规则层安全决策，包含 allow、redact、block、human_review 和原因。
- permission_scope: 用户可见账户、campaign、app、MMP 和字段级权限。
- delivery_policy: 当前阶段交付策略、禁止项、语言风格和客户可见限制。
- language_preference: 用户语言偏好、query 主语言和系统界面语言。

## 角色
你是 AdOps Copilot 的总控安全交付整理器。你只负责把已经完成安全规则检查的结果整理为内部可读交付卡，不负责重新判断权限、重新计算风险、重新计算置信度或生成客户可直接发送文本。

## 处理规则
① 若 `guard_decision.decision` 为 `block`，只输出拒绝或转人工说明，不展示 workflow_output 中的敏感细节。
② 若 `guard_decision.decision` 为 `allow_with_redaction`，必须删除或泛化 `guard_decision.redact_fields` 指定内容。
③ 若 `routing_final.risk_level_final` 为 high，必须 `requires_human_review=true`。
④ 所有确定性结论必须引用 `evidence_objects` 中的 evidence_id；无证据内容只能放入 `limitations` 或 `next_actions`。
⑤ 当前 PRD 不允许生成客户可直接发送回复；需要对外沟通时，只能提示 AM 基于内部摘要人工改写。

## 禁止
- 不得输出 raw postback URL、token、secret、用户级日志、合同、赔偿或客户承诺。
- 不得修改 `confidence_final`、`risk_level_final` 或 `guard_decision`。
- 不得把客户反馈问题改写成客户可见回复。
- 不得新增 workflow_output 和 evidence_objects 中不存在的事实。

## 输出 schema
{
  "schema_version": "delivery_guard_v1",
  "trace_id": "string|null",
  "delivery_type": "internal_diagnosis|knowledge_answer|clarification|refusal|human_review_required",
  "risk_level_final": "low|medium|high",
  "confidence_final": 0.0,
  "safe_summary": "string",
  "evidence_refs": [
    {"evidence_id": "string", "usage": "supports_summary|supports_limitation|supports_next_action"}
  ],
...
| 安全交付整理 | 规则引擎 + Qwen2.5-72B-Instruct | GLM-4-Plus、DeepSeek-V3、人工确认 | 与第 8 章一致，规则层先决定放行、脱敏、拒绝或转人工，模型只做内部表达整理 |
...
| 功能 | 说明 |
| --- | --- |
| 异常指标识别 | 判断 spend、impressions、clicks、installs、cvr、cpa、roi 哪个指标异常 |
...
| `{{routing_result_json}}` | 规则归一后的 `routing_final`，包含 intent、entities、risk_level_final、selected_workflow、tool_constraints | 确认任务类型、实体和可调用工具约束 |
...
## 输入上下文
- trace_context: 当前 trace、prompt 版本、模型、入口和时间，用于审计、回滚和成本归因。
- routing_result: 总控规则归一后的 intent、entities、risk_level_final、selected_workflow 和工具约束。
- user_query: 用户原始问题，用于判断诊断是否回应真实诉求。
- metric_results: 报表工具返回的指标序列、基线对比和漏斗数据；所有数值计算已由规则或 SQL 完成。
- account_status: 账户、预算、余额、投放状态、审核状态和配置状态。
- retrieved_knowledge: 已通过权限过滤和 rerank 的 SOP、指标口径或历史案例摘要。
- evidence_objects: 标准化证据对象；确定结论必须引用其中的 evidence_id。
- business_rules: 异常阈值、基线窗口、禁止动作、置信度公式和降级策略。
...
投放诊断 Prompt 不输出数值 `confidence`。模型只输出 `confidence_rule_inputs`、证据引用和缺失证据；workflow 根据下列规则计算 `confidence_final`，并传给 `delivery_guard_output.confidence_final`。
...
```json
{
  ...
  "missing_evidence": ["postback_summary", "mmp_report"],
  "confidence_rule_inputs": {
    "has_current_metric_data": true,
    "has_account_status": true,
    "has_key_funnel_metrics": true,
...
| --- | --- | --- |
| `{{trace_context_json}}` | trace_id、prompt_version、model、入口、时间 | 关联日志、评测和回滚 |
| `{{routing_result_json}}` | 规则归一后的 `routing_final`，包含 intent、entities、risk_level_final、selected_workflow、tool_constraints | 确认归因核对对象和可调用工具约束 |
...
| `{{postback_summary_json}}` | postback 成功、失败、延迟、拒收摘要 | 判断回调链路是否影响差异 |
...
| `postback_summary_json` | `get_postback_summary` | 查询聚合 success、delay、failed、reject，不传 raw URL 或用户级日志 | 缺失时 postback 只能标记 `not_supported_by_evidence` | `{"success_rate":"96%","delayed_count":830}` |
...
```json
{
  ...
  "postback_summary": {{postback_summary_json}},
...
## 输入上下文
- trace_context: 当前 trace、prompt 版本、模型、入口和时间，用于审计、回放和成本归因。
- routing_result: 总控规则归一后的归因核对 intent、实体、risk_level_final 和工具约束。
- user_query: 用户原始问题，用于判断解释是否回应平台与 MMP/客户后台差异。
- platform_report: 平台侧聚合指标、事件、时间窗口、时区和数据刷新时间。
- mmp_report: MMP 侧聚合指标、事件、时间窗口、时区和数据刷新时间。
- postback_summary: 聚合 postback 成功率、失败、拒收、延迟和重试摘要，不包含 raw URL 或设备级日志。
- attribution_policy_context: 已审核归因口径、窗口、去重、re-attribution 和隐私限制引用。
- event_mapping: 平台事件与 MMP/客户事件映射关系及审核状态。
- evidence_objects: 标准化证据对象；原因排序和排除项必须引用其中的 evidence_id。
- business_rules: 差异率公式、material threshold、必查项、置信度封顶和降级规则。

规则层传给 `delivery_guard` 的字段示例：
...
`confidence_final` 由 workflow 规则层计算，不由模型输出。模型只输出 `confidence_rule_inputs` 和证据解释，规则层按下列公式计算后传给 `delivery_guard_output.confidence_final`：
...
```json
{
  ...
  "postback_summary": {"success_rate": "96%", "delayed_count": 830, "failed_count": 12},
...
总控识别 knowledge_lookup
  ...
  -> 总控 Query 改写：英文语义 query、中文语义 query、实体 query
...
## 输入上下文
- trace_context: 当前 trace、prompt 版本、模型、入口和时间，用于审计、回放和成本归因。
- user_query: 用户原始知识问题，用于判断问题是否仍是纯知识查询。
- rewritten_queries: Query 改写环节生成的中英文语义 query、实体 query 和核查 query。
- selected_citations: RAG 经过权限过滤、source_type 白名单、rerank、去重和 topK 截断后的引用列表。
- source_type_scope: 当前回答允许使用的文档类型范围。
- language_preference: 用户语言偏好、query 主语言和界面语言，用于选择回答语言。
- citation_policy: 最低相关度、reviewed 状态、deprecated 过滤、冲突处理和引用数量上限。
...
知识查询 Prompt 不输出数值 `confidence`。模型只输出 `citation_coverage`、引用列表、限制说明和下一步建议；规则层按引用覆盖、最高相关度、reviewed 来源比例和时效性计算 `confidence_final`，并传给 `delivery_guard_output.confidence_final`。

引用置信度由规则层计算，不由模型输出。规则层可按引用覆盖、最高相关度、引用状态和冲突情况计算 `confidence_final`：
...
## 输入上下文
- eval_context: 评测 case、trace、prompt 版本、judge 模型和评测类型，用于追溯评测来源。
- scenario: 当前被评估任务类型，如 routing、performance_diagnosis、attribution_check 或 knowledge_lookup。
- user_query: 用户原始问题，用于判断被评估输出是否回应真实诉求。
- agent_output: 待评估智能体的原始结构化输出，不允许 Judge 自行重写后再评分。
- evidence_objects: 当次回答可见的证据对象，用于检查结论是否有 evidence_id 支撑。
- golden_answer: 黄金集标准答案、must_check、expected_reason 和 forbidden_output。
- safety_policy: 越权、敏感字段、客户承诺、无证据强答和高风险输出规则。
- scoring_rubric: 维度权重、阻塞项、通过阈值和人工复核规则。
...
```json
{
  ...
  "golden_answer": {"must_check": ["timezone", "attribution_window", "event_mapping", "postback_summary"]},
...
| RAG Query 改写器 | Qwen2.5-72B-Instruct | DeepSeek-V3、GLM-4-Plus、GPT-4o mini | 需要中英双语改写、实体保留和结构化输出；不生成答案，低温度运行 |
...
| 纯知识回答智能体 | DeepSeek-V3 + RAG 引用约束 | Qwen2.5-72B、GLM-4-Plus、GPT-4o mini | 中英知识回答需要表达质量和引用遵循；无引用不允许强答 |
| 总控安全交付整理器 | 规则引擎 + Qwen2.5-72B-Instruct | GLM-4-Plus、DeepSeek-V3、人工确认 | 规则层先决定放行、脱敏、拒绝或转人工，模型只做内部表达整理 |
...
4. Query 改写、知识回答和安全交付整理属于低温度结构化任务，默认使用国内通用轻量 LLM；召回、rerank、引用选择和脱敏仍由系统组件完成。
5. 所有高风险输出必须经过规则引擎和独立审核模型；安全交付 Prompt 不能绕过 `guard_decision`。
...
前端不直接消费 `workflow_output`，只按 `delivery_guard_output` 和证据引用状态渲染。状态文案需要让用户明确知道“当前结论是否完整、是否降级、是否需要人工确认”。

| 状态 | 触发条件 | 前端展示 | 用户可操作 |
| --- | --- | --- | --- |
| `ready` | `delivery_type=internal_diagnosis` 且 evidence_refs 非空，`requires_human_review=false` | 展示安全摘要、证据引用、下一步动作 | 采纳、继续追问、标记 Badcase |
| `human_review_required` | `requires_human_review=true` 或 `risk_level_final=high` | 展示内部摘要和人工确认提示，不展示客户可见话术 | 升级工单、分派 Owner、补充证据 |
| `partial_evidence` | 核心工具缺失、证据不足或 `limitations` 非空 | 展示“基于部分证据”的提示，突出缺失数据 | 补充 campaign/app/event/time range 或上传报表 |
| `tool_degraded` | 工具使用缓存、ETL 快照或人工上传数据 | 展示数据来源、快照时间和置信度下调原因 | 重试工具、等待刷新、人工确认 |
| `no_reliable_citation` | 知识查询无可靠引用或引用低于阈值 | 不生成确定答案，展示知识缺口和建议 Owner | 提交知识缺失、联系知识 Owner |
| `citation_conflict` | 引用之间存在口径冲突 | 展示冲突来源、生效时间和待确认项 | 转人工确认、标记 Badcase |
| `permission_blocked` | 工具或证据超出用户权限 | 只展示权限不足说明，不展示数据摘要 | 申请权限、联系管理员 |
| `out_of_scope` | SDK/API、素材审核、客户回复、操作变更等非当前范围 | 展示范围外说明和可转入的内部诊断/知识查询入口 | 记录需求、进入支持的 workflow |
| `system_error` | schema 校验失败、模型超时、工具注册表不可用 | 展示稍后重试和 trace_id | 重试、提交问题 |
...
| 工具 | 用途 | 输入 | 输出 | 权限 |
| --- | --- | --- | --- | --- |
| search_knowledge_base | 检索知识库 | query、source_type、locale、entity filters | SOP、口径、FAQ、历史案例引用 | 内部用户，按知识权限过滤 |
...
| get_platform_report | 查询平台侧归因/转化聚合报表 | account、campaign、app、event、time range、timezone | 平台侧聚合值、刷新时间、口径摘要 | AdOps/AM 按账户授权 |
| get_mmp_report | 查询 MMP 侧聚合报表 | app、campaign、event、time range、timezone、MMP | MMP 聚合值、刷新时间、口径摘要 | AdOps/AM 按客户和 MMP 授权 |
| get_postback_summary | 查询回调状态聚合摘要 | app_id、campaign_id、event、time range | 成功/失败/延迟/拒收摘要 | 技术支持/AdOps 限定字段 |
...
Evidence object 是证据源对象，不是最终前端展示对象。其 `confidence` 表示单条证据的来源可信度或新鲜度评分；最终回答置信度统一使用 `delivery_guard_output.confidence_final`。
...
降级优先级为：实时 API > 有效缓存 > ETL 快照 > 人工上传报表 > 只输出待补充信息。任何降级路径都必须写入 Trace，并在 `delivery_guard_output` 渲染的内部诊断卡上显示数据新鲜度和置信度。
...
Prompt 与 workflow 端到端回归测试说明不作为 PRD 主体内容，独立维护在 `03_prd/eval/AdOps_Copilot_Prompt_E2E_验证说明.md`。
...
| 小流量灰度 | 5-10 名资深 AdOps | 验证真实问题采纳率 | 采纳率 60% 以上，越权率 0 |
...
```json
{
  ...
  "allowed_params": {
    ...
    "compare_with": "baseline_time_range",
...
```json
{
  ...
  "required_outputs": ["platform_value", "timezone", "data_freshness", "metric_definition_version"],
  "permission_check": ["account_scope", "role", "region"],
  "timeout_ms": 8000
}
```
...
```json
{
  ...
  "required_outputs": ["mmp_value", "timezone", "data_freshness", "attribution_window"],
  "permission_check": ["account_scope", "mmp_access"],
  "timeout_ms": 8000
}
```

### 15.2.4 Postback 摘要查询
...
```json
{
  ...
  "blocked_params": ["raw_postback_url", "token", "secret", "device_id", "user_id"],
  "required_outputs": ["success_rate", "failed_rate", "delayed_rate", "reject_rate", "data_freshness"],
  "permission_check": ["postback_summary_read", "account_scope"],
  "timeout_ms": 8000
}
```
...
+ Σ(Rerank token / 1,000,000 × Rerank token 单价)
+ Σ(Rerank 请求数 × Rerank request 单价)
+ Σ(外部工具调用次数 × 工具单位成本)
```

Rerank 计费口径按供应商配置二选一或分别记录：国内/云厂商模型常按输入 token 估算，部分海外服务或向量数据库集成按 request 计费。线上成本看板必须同时记录 `rerank_tokens` 和 `rerank_requests`，但结算时按 `price_config_version` 选择实际适用口径，避免重复计费。
...
| Embedding 查询 | Alibaba `text-embedding-v3` / BAAI `bge-m3` 按线上价格配置估算，示例按约 ¥0.52 / 1M tokens | OpenAI `text-embedding-3-large`，按 $0.13 / 1M tokens 估算 |
| Rerank | Alibaba `gte-rerank` / BAAI `bge-reranker-v2-m3` 按 token 价格配置估算，示例按约 ¥0.83 / 1M tokens | Cohere Rerank 3.5 / Pinecone 集成，按 $2 / 1K requests 估算 |
...
检索成本 = Embedding token / 1,000,000 × Embedding 单价 + Rerank token / 1,000,000 × Rerank token 单价 + Rerank 请求数 × Rerank request 单价
...
5. 线上必须按 `workflow_type`、`model_name`、`prompt_version`、`input_tokens`、`output_tokens`、`cached_input_tokens`、`embedding_tokens`、`rerank_tokens`、`rerank_requests`、`tool_retry_count` 和 `price_config_version` 记录成本，避免只能用估算回答业务 ROI。
