...
| 3. 场景智能体拆解 | 生成核查计划：时区、归因窗口、事件映射、去重、postback、刷新延迟、渠道映射 | 证据需求列表、工具计划 | 指标计算和差异比例由规则或 SQL 计算，模型不自由计算 |
...
```json
{
  ...
  "output": {
    ...
    "risk_level": "medium",
    "missing_fields": [],
    "tool_constraints": ["get_campaign_metrics", "retrieve_sop", "search_similar_cases"],
...
本需求对应任务一“意图识别与 Query 改写”。Prompt 目标不是直接回答用户问题，也不是替代场景智能体完成完整排查拆解，而是把自然语言问题转成可执行、可审计、可权限校验的路由结果和调用约束。
...
| `{{available_tools}}` | 工具名、参数 schema、权限要求、超时策略 | 生成候选工具范围和调用约束 |
...
角色：
你是 AdOps Copilot 投放归因排障助手的任务总控智能体，负责识别投放和归因问题的意图、实体、风险、权限边界和场景路由。
...
3. 判断信息是否足够。
4. 判断用户是否可能需要受限数据。
5. 生成候选工具范围和调用约束，供场景智能体细化排查步骤。
6. 不回答最终业务结论，不替代场景智能体完成完整诊断。
...
| 复杂路由编排 | OpenAI GPT-4o | DeepSeek-R1/DeepSeek-Reasoner、Qwen2.5-Max | 多意图、多实体、多场景分流时更稳，可通过模型路由控制成本 |
...
OpenAI 模型作为首选方案的前提是企业具备合规可用的海外主体、数据处理协议和跨境数据处理边界。如果实际公司主体、客户合同或数据合规要求不允许使用海外模型，默认切换为中国厂商或私有化可部署方案，PRD 的模型接口必须保持可替换。
