...
本需求对应任务一“意图识别与 Query 改写”。Prompt 目标不是直接回答用户问题，而是把自然语言问题转成可执行、可审计、可权限校验的任务计划。
...
本需求对应任务四“Judge AI 评估”。Judge AI 属于 Badcase、评测和版本治理链路，负责在输出交付前检查证据、完整性、安全和可执行性，并把阻塞问题转化为 Badcase 或人工复核任务。
