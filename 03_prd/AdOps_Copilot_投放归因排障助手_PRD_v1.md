...
| `{{routing_result_json}}` | 总控归一后的 intent、entities、risk_level、selected_workflow | 限制 query 改写场景和实体范围 |
...
- routing_result: 总控规则归一后的 intent、entities、risk_level 和 selected_workflow。
...
- routing_result: 总控规则归一后的 intent、entities、risk_level、selected_workflow 和工具约束。
...
- routing_result: 总控规则归一后的归因核对 intent、实体、风险等级和工具约束。
...
知识查询 Prompt 不输出数值 `confidence`。模型只输出 `citation_coverage`、引用列表、限制说明和下一步建议；规则层按引用覆盖、最高相关度、reviewed 来源比例和时效性回写 `confidence_final`。
