...
| 国内厂商评测对照模型 | 作为 Judge AI 对照、客户话术离线评审、模型 A/B | 阿里云百炼 `qwen-max-2025-01-25`、DeepSeek `deepseek-chat` / `deepseek-reasoner`、智谱 `GLM-4-Plus` | Anthropic 因企业主体和采购约束暂不考虑；国内模型用于降低单模型自评偏差，并验证中文内部场景质量 |
| 轻量回复与摘要模型 | 低风险 FAQ、会话标题、内部摘要 | OpenAI `gpt-4o-mini-2024-07-18` | 成本和延迟优先，输出受 Schema 和审核规则约束 |
| Embedding 模型 | 文档向量化、历史案例向量化、查询向量化 | OpenAI `text-embedding-3-large`，国内云备选为阿里云百炼 `text-embedding-v3`，成本敏感场景可用 `text-embedding-3-small` | 中英文混合知识、广告术语、长文档片段需要较强语义表示；国内模型用于企业采购或数据出境受限场景 |
| 自托管 Embedding 备选 | 客户数据不能出域、私有化部署 | BAAI `bge-m3` | 支持多语言、长文本、多粒度检索，适合作为数据出域受限时的备选 |
| 重排序模型 | 提升召回准确性，减少相似但错误场景命中 | Cohere `rerank-3.5`，国内云备选为阿里云百炼 `gte-rerank` | 支持多语言和企业复杂文档，对中英混合 SOP、工单、政策文档更稳；国内模型用于采购和数据流转约束场景 |
...
- Anthropic 系列模型因企业主体、采购和合规约束暂不进入候选池，也不作为离线评测或备份模型。
- 国内厂商模型需要纳入同阶段评估，重点验证中文内部协作、企业采购可行性、私有化或国内云部署能力，以及对英文客户回复的稳定性。
...
| 阿里云百炼 / 通义千问 | `qwen-max-2025-01-25` / Qwen2.5-Max | 中英双语能力强，适合中文内部协作、复杂文本理解和国内企业采购；API 形态适合生产接入评估 | 英文客户回复自然度、海外广告术语和工具调用稳定性需用真实场景评测 | 作为国内厂商首选对照模型，用于离线 Judge AI、中文内部诊断评测和备份方案评估 |
| 阿里通义千问开源 | `Qwen2.5-72B-Instruct` | 开源权重、可自托管，适合数据不出域和私有化评测 | 运维成本、推理成本、结构化输出稳定性需压测 | 作为自托管 LLM 候选，不进入 V1.0 默认在线链路 |
| DeepSeek | `deepseek-chat`，对应 DeepSeek-V3 | 推理和生成性价比高，适合复杂诊断、中文内部问答和批量离线评测 | 企业合规、服务稳定性、客户英文话术质量需单独验证 | 作为成本敏感的复杂诊断备选和离线评测对照 |
| DeepSeek | `deepseek-reasoner`，对应 DeepSeek-R1 | 适合复杂推理、归因口径冲突、日志链路判断等二次核查任务 | 推理延迟、输出长度、结构化稳定性和客户可见表达需严格门禁 | 作为 `o3-mini` 的国内推理备选，不走每轮必经链路 |
| 智谱 AI | `GLM-4-Plus` / `GLM-4-Air` | 国内企业 API 生态成熟，中文理解和轻量任务覆盖较好 | 英文客户回复、海外广告行业术语和多模态能力需评测 | 作为总控、内部摘要、离线评测候选 |
| Moonshot AI | `moonshot-v1-128k` / `moonshot-v1-32k` | 长上下文能力适合长文档、历史工单和政策材料离线分析 | 结构化输出、工具调用和线上稳定性需验证 | 作为长文档离线解析和知识库治理候选 |
| 百度智能云千帆 | ERNIE 4.0 Turbo 系列 | 国内企业云、权限和采购体系较成熟，中文企业知识场景适配度高 | 英文客户回复质量、海外广告术语和开放工具链适配需评测 | 作为企业采购备选，不进入 V1.0 默认在线链路 |
| 腾讯混元 | `Tencent-Hunyuan-Large` / 混元大模型 API | 国内云生态和长文本能力可评估，适合企业知识库与中文内部场景 | 海外英文话术、多模态、结构化输出和成本需评测 | 作为国内云备选和离线 Benchmark 候选 |
...
| 阿里云百炼 | `text-embedding-v3` | 支持多语种文本向量化，企业采购和国内云部署路径更清晰 | 与 OpenAI 向量空间不可混用，迁移需重建索引和回归评测 | 国内云部署或数据出境受限时的优先备选 |
...
| 阿里云百炼 | `gte-rerank` | 国内云可采购，适合中文和中英混合知识库的重排序评估 | 英文客户场景、海外广告术语和延迟需用双语检索集验证 | 国内云部署或供应商约束场景的优先备选 |
...
| 投放诊断智能体 | OpenAI `gpt-4o-2024-08-06` | 需要结合报表、指标公式、历史案例生成诊断 | 需要跨数据源综合和广告业务语义理解 | 简单指标解释用 `gpt-4o-mini`；复杂冲突用 `o3-mini`，国内备选为 `deepseek-reasoner` |
| 归因核对智能体 | OpenAI `gpt-4o-2024-08-06` + 规则核对清单 | MMP 与平台数据不一致、SKAN/SAN/Postback 问题 | 需要固定核对顺序、英文客户解释和谨慎结论 | 高复杂度口径冲突用 `o3-mini`，国内备选为 `deepseek-reasoner` |
...
| 客户回复智能体 | OpenAI `gpt-4o-2024-08-06` | 生成英文邮件、工单回复、会议纪要式说明 | 对外英文质量、保守表达和结构化解释要求高 | 低风险短回复可用 `gpt-4o-mini`；离线用 `qwen-max-2025-01-25`、`GLM-4-Plus`、`deepseek-chat` 对照评审 |
| 回复审核智能体 | 规则引擎 + OpenAI `gpt-4o-mini-2024-07-18` | 客户回复发送前、复制前 | 高频审核，主要识别敏感信息、过度承诺、无证据结论 | 高风险回复升级到 `gpt-4o-2024-08-06` 或人工 |
| Judge AI | 阿里云百炼 `qwen-max-2025-01-25` + DeepSeek `deepseek-chat` / `deepseek-reasoner` + OpenAI `gpt-4o-2024-08-06` 抽样对照 | 离线评测、Prompt A/B、Badcase 归因 | 避免主生成模型自评偏差，同时验证国内模型在中文内部场景和复杂推理场景的可用性 | SME 抽检作为最终仲裁 |
...
| [Qwen2.5-Max 官方说明](https://qwenlm.github.io/blog/qwen2.5-max/)、[通义千问 Qwen2.5 模型卡](https://huggingface.co/Qwen/Qwen2.5-72B-Instruct)、[阿里云百炼模型列表](https://www.alibabacloud.com/help/en/model-studio/models)、[阿里云百炼模型计费](https://www.alibabacloud.com/help/en/model-studio/model-pricing) | 评估 Qwen 系列生成、Embedding、Rerank 和国内云接入能力 |
| [DeepSeek-R1 发布说明](https://api-docs.deepseek.com/news/news250120)、[DeepSeek-V3 GitHub](https://github.com/deepseek-ai/DeepSeek-V3)、[DeepSeek API 模型文档](https://api-docs.deepseek.com/quick_start/pricing) | 评估 DeepSeek 生成和推理模型作为复杂诊断、Judge AI、成本敏感备选 |
| [智谱开放平台模型价格与列表](https://open.bigmodel.cn/pricing) | 评估 GLM 系列作为国内企业 API、轻量路由和内部中文摘要备选 |
| [Moonshot/Kimi 模型价格](https://platform.kimi.ai/docs/pricing/chat-v1)、[Moonshot Open Platform 文档](https://platform.kimi.ai/docs/introduction) | 评估 Kimi/Moonshot 长上下文模型作为长文档离线解析备选 |
| [百度智能云千帆模型服务文档](https://cloud.baidu.com/doc/WENXINWORKSHOP/index.html)、[ERNIE 4.0 Turbo 发布信息](https://ir.baidu.com/news-releases/news-release-details/baidu-launches-ernie-40-turbo-accelerating-foundation-model) | 评估文心系列作为国内企业云备选 |
| [腾讯混元开源模型仓库](https://github.com/Tencent/Tencent-Hunyuan-Large)、[腾讯云混元大模型文档](https://cloud.tencent.com/document/product/1729) | 评估混元系列作为国内云和长文本候选 |
...
| 离线话术对照 | 阿里云百炼 `qwen-max-2025-01-25`、DeepSeek `deepseek-chat`、智谱 `GLM-4-Plus` | 用于评测英文表达、保守程度和客户可读性，不作为线上强依赖；不可采购模型不纳入评测池 |
...
V1.0 默认使用 OpenAI `text-embedding-3-large` 作为主 Embedding 模型，低价值文档和离线预筛可使用 `text-embedding-3-small`。如企业采购、区域部署或数据出境要求优先使用国内云服务，则评估阿里云百炼 `text-embedding-v3`。数据不能调用外部 API 或需要私有化部署时，使用 BAAI `bge-m3` 作为备选。
...
V1.0 默认使用 Cohere `rerank-3.5` 对召回结果进行重排序，结合元数据规则提升权威来源、最新版本和目标地区匹配度。如企业采购或区域部署优先使用国内云服务，则评估阿里云百炼 `gte-rerank`。数据不能调用外部 Rerank API 时，使用 BAAI `bge-reranker-v2-m3` 自托管。
...
用户 Query
↓
OpenAI gpt-4o-mini-2024-07-18 进行 Query 改写、双语扩展和实体抽取；国内云备选为 qwen-plus 或 GLM-4-Air
↓
权限过滤和元数据过滤
↓
关键词召回：错误码、事件名、平台名、指标名、Campaign ID
↓
向量召回：OpenAI text-embedding-3-large 检索语义相似 SOP、历史案例、政策说明；国内云备选为阿里云 text-embedding-v3
↓
结构化查询：指标字典、归因口径、错误码表
↓
Cohere rerank-3.5 重排序：按场景相关性、来源权威性、版本新鲜度排序；国内云备选为阿里云 gte-rerank
↓
Top-K 引用压缩
↓
传入子智能体生成答案
...
| 评测对象 | 候选 | 评测集 | 决策标准 |
|---|---|---|---|
| 总控模型 | `gpt-4o-mini-2024-07-18`、`qwen-plus`、`GLM-4-Air`、规则分类器 | 意图识别集、双语检索集、安全集 | 意图准确率、JSON 稳定性、P95 延迟、单次成本 |
| 复杂诊断模型 | `gpt-4o-2024-08-06`、`qwen-max-2025-01-25`、`deepseek-chat`、`GLM-4-Plus`、`gemini-1.5-pro-002` | 投放诊断集、归因核对集、SDK 排障集 | 原因命中率、证据支撑率、英文解释质量、工具调用稳定性 |
| 深度推理模型 | `o3-mini`、`deepseek-reasoner`、`gpt-4o-2024-08-06` | 复杂归因冲突集、复杂日志集 | 多步骤推理正确率、低置信度识别率、延迟和成本 |
| 长文档处理模型 | `moonshot-v1-128k`、`gemini-1.5-pro-002`、`qwen-max-2025-01-25` | 长 SOP、MMP 文档、SDK 文档、历史工单集 | 关键信息抽取完整率、引用准确率、上下文成本 |
| Embedding | `text-embedding-3-large`、`text-embedding-3-small`、阿里云 `text-embedding-v3`、`embed-multilingual-v3.0`、`voyage-3-large`、`jina-embeddings-v3`、`bge-m3` | 双语检索集、历史案例集、审核政策集 | Recall@K、MRR、跨语言命中率、索引成本、数据出域合规 |
| Rerank | `rerank-3.5`、阿里云 `gte-rerank`、`bge-reranker-v2-m3`、元数据规则重排 | 双语检索集、相似错误场景集 | Top-3 命中率、错误引用率、延迟、私有化可行性 |
| 客户回复模型 | `gpt-4o-2024-08-06`、`qwen-max-2025-01-25`、`deepseek-chat`、`GLM-4-Plus`、`gpt-4o-mini-2024-07-18` | 客户回复集、安全集 | 英文可发送率、保守程度、敏感信息泄露率、改写成本 |
...
| 是否允许将脱敏后的客户问题、日志摘要和工单片段发送到 OpenAI、Cohere、阿里云百炼、DeepSeek、智谱、Moonshot、百度千帆、腾讯混元等外部模型 API | 决定是否能使用默认模型选型，或必须切换到 BGE-M3 / BGE Reranker 等自托管方案 |
| 海外业务是否存在 EU、US、东南亚等区域数据驻留要求 | 决定模型服务区域、日志存储区域和向量库部署区域 |
| 是否已有企业级 OpenAI / Cohere / 阿里云百炼 / DeepSeek / 智谱 / Moonshot / 百度千帆 / 腾讯混元合同、DPA 和数据保留条款 | 决定供应商准入、审计要求和上线节奏 |
| 当前因企业主体和采购约束排除的海外模型厂商是否可能在后续阶段放开 | 决定后续评测池是否扩展，或继续以可采购模型和国内厂商模型为主 |
