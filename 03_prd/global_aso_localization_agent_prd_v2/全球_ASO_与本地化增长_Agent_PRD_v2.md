...
全球 ASO 与本地化增长并不是一个固定流程，而是至少包含三类不同任务：新 App 或新市场的 0→1 商店页搭建、围绕新定位或产品版本的整体改版、已有稳定商店页的持续精细化优化。三类任务的目标、可接受改动范围和验证方式不同：前两类通常需要标题、描述、截图叙事与本地化表达协同调整，先建立一个可用基线；第三类才更适合在流量和数据条件允许时控制变量做实验。

当前项目更值得解决的是以下实际问题，而不是抽象复用某个行业案例的矛盾数量：

- 市场信号不等于确定事实。评论有自选择偏差，关键词量级和竞品表现可能来自第三方估算，竞品采用某种表达也不能证明该表达导致了转化。运营需要同时看到来源、时间、口径和局限。

- 本地化交付物彼此耦合。关键词、标题、描述、截图叙事、视觉风格和产品内真实能力共同构成用户理解；只优化单个文本字段，可能造成搜索表达、卖点承诺与素材呈现不一致。

- 任务阶段决定验证方法。0→1 或大版本改版需要先形成协调一致的新基线，不能为了可归因而拆成许多无法独立上线的小改动；稳定版本的精细化迭代才适合优先考虑单一主变量实验。

- 团队按交付物协作，但增长决策需要按假设协作。ASO、本地化、设计、数据和发布人员各自完成文件或页面，原始用户需求、证据、版本目标和验证条件容易在交接中丢失。

- 结果反馈稀疏且受干扰。流量结构、广告活动、产品版本、评分评论、季节性和平台变化都可能影响结果；低流量场景甚至无法在合理时间内得到可靠实验结论。

因此，本项目的机会不是把所有 ASO 调整都变成单变量实验，而是让 Agent 先识别任务模式，再生成匹配该模式的版本方案和验证路径：该建立新基线时建立新基线，具备条件时再做控制实验，条件不足时明确证据局限并保留人工判断。
...
| 验证方式错配   | 0→1 或大版本调整被强行拆成单变量实验，或精细化迭代一次修改过多变量 | 未识别任务模式，Version、Hypothesis 与 Validation 状态不完整 |
