...
| 类型 | 判断标准 | 后续动作 |
| --- | --- | --- |
| PoC 成功 | 能生成完整、可追溯、可编辑的版本方案；关键引用和产品事实通过人工抽检；三类任务得到合理验证路径 | 进入任务对照和交互 MVP 评审 |
| 任务价值成立 | 相对现有流程改善准备时长、证据完整或跨角色审核，且人工维护与审核成本未抵消收益 | 申请受控试点；阈值在基线后冻结 |
| 业务价值成立 | 真实试点在可比窗口或合格实验中得到可解释结果，且安全与质量未退化 | 评估扩大市场、App 或连接器 |
| 产品假设失败 | 用户只需要单点查询/翻译，或 Agent 辅助对效率、质量和协作没有实质改善 | 收缩为 Copilot、流程工具或停止完整 Agent 方向 |
| 技术 / 数据失败 | 数据许可、引用质量、多语质量、成本或时延无法满足最低任务要求 | 缩小数据和市场范围，改用规则/搜索/人工，必要时停止 PoC |
| 立即停止或回滚 | 发生越权取数或动作、未经确认发布、虚构高风险产品事实、严重文化/品牌错误，或人工队列无法承接降级任务 | 阻断外部动作、回滚版本、转人工并进入事故与 Bad Case 流程 |

Gate A 当前状态：待用户评审，尚未正式冻结。问题、AI/人工/规则边界、非目标和 MVP 基线已形成方案；进入第 6—8 章前仍需确认首个 App 品类、两个具体市场、MVP 是策略包还是可交互 Demo，以及可使用的数据和母语审核资源。
