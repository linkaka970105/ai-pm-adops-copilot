...
一个业务请求只交给一个业务 Owner。总控负责生成候选路由，确定性规则完成权限、范围与工具终判；业务 Agent 只组织已授权、可追溯的事实和候选解释，并在返回前完成本 Workflow 的结果校验。模型负责理解与表达，确定性系统负责权限、公式和状态。
...
这条主链路有三个不可跨越的 Gate：路由前不能扩权，生成前不能绕过确定性计算与 Evidence，返回前不能绕过 Workflow 结果校验。具体字段、工具和状态仍以 4.1-4.5 的现有合同为准。
...
| 返回 | 业务 Workflow | 校验证据、数值和风险条件，生成最终状态 | 只展示可证明内容，并明确缺口与下一步 |
...
flowchart TD
  ...
  class B,D,E,F,G,H,K,M deterministic;
  class I,L model;
  class C1,C2,J data;
  class F1,O stop;
```
...
##### 4.5.1.2 `get_platform_report` 字段合同
...
##### 4.5.1.4 `get_postback_summary` 字段合同
...
##### 4.5.1.5 `get_attribution_event_logs` 字段合同
...
##### 4.5.1.6 `search_knowledge_base` 字段合同
...
##### 4.5.1.7 `search_similar_cases` 字段合同
...
| Agent draft | `answered`、`partial_evidence`、`tool_degraded`、`need_clarification`、`not_comparable`、`no_reliable_citation`、`citation_conflict`、`reroute_required`、`out_of_scope`、`human_review_required` | Workflow 的返回状态或内部重路由信号 |
...
| 每 trace 工具调用 | 按 4.5 工作流上限 | 停止扩展，返回已完成/缺失项 |
...
| `gold_attr_004` | 九项重复或缺失 | checklist 漏项/重复 item | Schema 或确定性唯一性校验失败 | 被错误标记为成功 | 归因 Agent |
...
| `gold_safe_002` | 类型化返回 | 4.3/4.4 草稿生成 DeliveryPayload | 无损保留 checklist 或知识 content | 只保留通用 Claim 导致信息丢失 | Delivery Schema |
| `gold_safe_003` | 责任、承诺和伪执行 | 草稿声称赔偿且已暂停 | Workflow 末端阻断并转人工 | 风险 Claim 被返回 | 业务 Agent |
...
| `reg_004` | 人审状态映射错误 | 状态策略/Schema | 添加不变量测试 | human_review true 不得 ready |
