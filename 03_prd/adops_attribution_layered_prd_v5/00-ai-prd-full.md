...
| 证据 | `evidence_objects`、`claims`、`verification_events` | 证据存储 | 是 | Claim 直接引用 Evidence ID；确认因果另需 Verification Event |
...
**[证据引用、权限与风险审核]（Evidence Reference, Permission & Risk Review）：** 通过确定性策略核验 Claim 引用的 Evidence ID、租户/对象/时间范围、数字、Derivation、Verification Event、可见权限和风险词/字段，阻断无合法引用断言、责任归属、客户承诺、写操作、敏感信息及注入残留；通过校验的 `proposed_evidence_ids` 才投影为 `allowed_payload.evidence_refs`，本节点不调用模型复判语义支持关系。
...
| 证据引用、权限与风险审核 | 不调用 | 不调用 | 不调用 | Evidence ID、范围、数字、Derivation、Verification Event、权限与风险策略校验 |
...
JSON Schema 之外仍需代码不变量：时间窗 start < end；查询涉及 spend/revenue 等货币指标时 `currency` 必须为非空 ISO 4217，纯计数查询可为 `null`；工具名与 result 类型匹配；`status=permission_denied` 时 result=null；`source_type=reviewed_case` 只能补充候选检查方向且不能成为候选原因的唯一证据；`derived_fact` 必须有 Derivation；`confirmed_cause` 必须有有效 VerificationEvent；Delivery 严格按 4.6.4 首个命中规则裁决，只有未命中更高优先级条件时 `requires_human_review=true` 才映射为 `human_review_required`；所有最终候选原因必须通过 Evidence ID、范围、数字与 Claim 类型校验；Evidence 的 tenant/policy/auth snapshot 必须与当前请求一致。`EvidenceClaimBundle` 还必须由代码验证引用完整性：Claim.proposed_evidence_ids、Derivation.input_evidence_ids 与 VerificationEvent.verification_evidence_ids 全部存在于同一 bundle；VerificationEvent 的 claim ID、tenant、对象 scope 与时间窗必须和 Claim/当前请求一致，禁止跨 bundle、跨租户、跨 Campaign 或跨时间范围拼接合法 Evidence。
