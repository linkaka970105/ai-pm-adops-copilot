...
**系统定位：** 请求路由器与范围守门员。质量优先级为“路由正确 > 权限与范围正确 > 低延迟”，一句话记忆是“模型只产候选，规则决定去哪、能不能去”。

- 把当前自然语言转换为候选意图、候选字段、风险信号和候选工具意图；不把模型判断包装成最终业务事实。
- 读取页面结构化上下文和服务端 `confirmed_conversation_slots`，严格区分 `candidate`、`confirmed` 与 `conflict`。
- 由确定性规则结合 Intent Registry、Slot Schema、Tool Registry、权限与产品范围生成唯一 `route_final`。
- 对写操作、客户承诺、责任归属、敏感数据与范围外请求阻断或转人工。
- 创建 `trace_id`，审计来源、模型/Prompt/Schema/规则版本、路由、耗时、Token 和停止原因。
- 不执行工具，不计算指标，不判断根因，不生成业务答案。

#### 4.1.2 输入、上下文与输出

| 类型 | 字段 | 来源 | 必需 | 说明 |
| --- | --- | --- | --- | --- |
| 原始问题 | `current_user_query` | 用户 | 是 | 保留原文；LLM 只从这里提取新的自然语言候选字段 |
| 页面上下文 | `page_context` | 前端页面 Schema | 是（可为 `null`） | 有页面时直接传 Campaign、平台、筛选器等结构化值，不需要 LLM 提取；无页面入口显式传 `null` |
| 已确认槽位 | `confirmed_conversation_slots` | 服务端会话状态 | 否 | 仅含经规则校验或用户确认的结构化字段；自由文本摘要不得替代槽位状态 |
| 近期对话 | `recent_dialogue` | 会话服务 | 否 | 只传必要有限轮次并保留 `turn_id`，不把整个历史自由总结后当事实 |
| 配置 | `intent_registry` | 配置中心 | 是 | 允许意图、必填字段、唯一 Owner 与版本 |
| 配置 | `slot_schema` | 配置中心 | 是 | 槽位类型、枚举、来源与校验规则 |
| 配置 | `tool_registry_summary` | 工具网关 | 是 | 只给模型候选名称和用途摘要，不授予工具调用权 |
| 安全 | `user_auth_context` | IAM/租户服务 | 是 | 仅供确定性权限终判；敏感权限细节不交给模型回显 |
| 系统 | `request_time`、`request_timezone`、`current_phase` | 服务端 | 是 | Date Resolver、范围与阶段规则使用 |
| 候选输出 | `route_candidate_v3` | 总控模型 | 是 | 候选意图、候选槽位、冲突、模型缺字段报告、风险、工具与澄清候选 |
| 终判输出 | `route_final_v2` | 确定性规则层 | 是 | 唯一 Owner、最终必填字段、权限/范围、允许工具、预算与工作流状态 |

`conversation_slots` 是服务端维护的结构化会话状态，不是历史对话的自由文本总结：

| 状态 | 含义 | 写入者 | 可否用于最终路由 |
| --- | --- | --- | --- |
| `candidate` | 模型刚从当前问题提取的候选值 | 模型只能写 `slot_candidates` | 否，需校验或确认 |
| `confirmed` | 已通过规则校验或用户明确确认 | 会话状态服务 | 是 |
| `conflict` | 当前输入、页面或历史 confirmed 值不一致 | 确定性冲突检测 | 否，必须澄清 |

关键槽位复用统一字段：`field`、`value`/`value_candidate`、`raw_value`（自然语言来源时必填）、`source`、`origin_source`（confirmed 状态保留原始来源）、`evidence_quote`（自然语言来源时必填）、`observed_at`、`confirmed_at`、`turn_id`、`schema_version`。对模型输入，历史确认值的 `source=confirmed_conversation_slot`，同时用 `origin_source=user_query|page_context` 保留来源；页面值不能伪装成用户原话，历史值不能伪装成当前输入。

#### 4.1.3 详细 Pipeline

1. **请求接入与上下文收集：** 输入原始问题、页面结构化上下文、服务端 confirmed slots 与受控近期对话；创建 `trace_id`，保留原始输入、来源、请求时间/时区和权限快照 hash。产出不可变请求上下文；输入损坏则停止为 `system_error`。**LLM：否；RAG：否；只读业务工具：否；确定性规则：是。**
2. **意图识别与候选字段提取：** 输入上一步上下文及三个注册表摘要；模型提取候选意图、槽位、冲突、缺失字段报告、风险、候选工具和澄清候选。产出 `route_candidate_v3`；空响应或 Schema 不合法限次重试，仍失败则模板澄清/转人工。**LLM：是；RAG：否；只读业务工具：否；确定性规则：仅输入裁剪与输出校验。**
3. **字段校验与会话状态合并：** 输入候选、页面字段和 confirmed slots；校验 ID、数字、币种、时区、平台、MMP、事件枚举，由 Date Resolver 解析相对时间，再合并可确认值。模型 candidate 不能直接升为 confirmed；当前输入与页面 Campaign、历史 confirmed slot，或时区/MMP/事件/时间范围冲突时写入 `slot_conflicts` 并停止到澄清。**LLM：否；RAG：否；只读业务工具：否；确定性规则：是。**
4. **权限、范围与路由终判：** 输入校验后的槽位、风险信号、注册表、权限和阶段规则；检查必填字段、对象权限、产品范围、工具白名单、预算与风险，生成 `route_final_v2`。模型候选不得覆盖规则终判；字段不足澄清，权限/范围失败阻断。**LLM：否；RAG：否；只读业务工具：否；确定性规则：是。**
5. **任务处置与执行审计：** 输入 `route_final_v2`；可执行任务只交唯一 Workflow Owner，字段不足澄清，越权/范围外阻断，高风险转人工；记录候选/终判、模型/Prompt/Schema/规则版本、耗时、Token、Owner、停止原因与 Badcase。**LLM：否；RAG：否；只读业务工具：否；确定性规则：是。**

**事实保真不变量：** 节点 1—3 共同保留 ID、数字、币种、时区、平台、MMP、事件、`raw_value`、证据原文和来源，不再单列成一个业务节点。模型只提取“昨天”“上周”等原始表达及候选时间语义；最终 `start_inclusive/end_exclusive` 由 Date Resolver 根据 `request_time + request_timezone` 计算，模型不得自行决定绝对日期。

#### 4.1.4 AI 模型选型与理由

项目时间固定为 **2024 年末至 2025 年初**。价格仅是当时公开快照，用于 PoC 成本测算，并非长期生产事实；上线前还要冻结供应商报价与合同版本。Qwen 快照后缀即本方案采用的公开版本日期：Turbo 为 2024-09-19，Plus 为 2024-12-20；阿里云 2024-09-19 生效的[官方降价公告](https://help.aliyun.com/zh/model-studio/qwen-model-billing-notice)给出 Qwen Turbo/Plus 系列实时调用标准价约 `¥0.3/¥0.6` 与 `¥0.8/¥2`/百万输入输出 Token，[官方版本清单](https://help.aliyun.com/zh/model-studio/rate-limit)可核对快照名。DeepSeek [V3 发布公告](https://api-docs.deepseek.com/zh-cn/news/news1226/)发布于 2024-12-26，[官方更新日志](https://api-docs.deepseek.com/zh-cn/updates)确认 `deepseek-chat` 当日升级为 V3，首发优惠价约为缓存输入 `¥0.1`、非缓存输入 `¥1`、输出 `¥2`/百万 Token（公告同时给出 2025-02-08 后 `¥0.5/¥2/¥8`）。OpenAI [Structured Outputs 发布页](https://openai.com/index/introducing-structured-outputs-in-the-api/)与[缓存定价公告](https://openai.com/index/api-prompt-caching/)记录 GPT-4o 2024-08-06、GPT-4o mini 2024-07-18 以及约 `$2.50/$10`、`$0.15/$0.60`/百万输入输出 Token。

| 定位 | 具体模型/版本 | 主要能力与适用任务 | 限制、成本与切换条件 |
| --- | --- | --- | --- |
| 国内常规主模型 | `qwen-turbo-2024-09-19` | 高频短输入的意图、实体、缺字段、风险语义与候选工具；Qwen Turbo/Plus 系列已有[Function Calling 官方支持](https://help.aliyun.com/zh/model-studio/qwen-function-calling) | 低成本低延迟；输出仍经 JSON Schema Validator，不能把 Function Calling 等同严格 Schema 保证 |
| 国内复杂歧义升级 | `qwen-plus-2024-12-20` | 字段完整但仍有多种合理意图、多意图依赖、复杂中英文广告术语和省略表达 | 仅 Turbo 无法形成唯一候选时升级；缺字段、权限、冲突、范围/写操作/承诺/责任/敏感请求不得靠升级解决 |
| 国内供应商备选/A-B | `DeepSeek-V3`（当时 API 名称 `deepseek-chat`） | 国内备选与离线 A/B；Qwen 在广告语义上持续出现同类 Badcase 时进入候选；官方 2024-07 [API 更新](https://api-docs.deepseek.com/news/news0725/)已提供 JSON Output 与 Function Calling | [JSON Output 官方说明](https://api-docs.deepseek.com/guides/json_mode/)提示需在 Prompt 中明确 JSON 与示例，且可能返回空内容；它不等于严格 JSON Schema，需限次重试和重新校验，不进入每次请求串行链 |
| 国外常规质量基准 | `gpt-4o-mini-2024-07-18` | 脱敏离线评测意图、实体与 Structured Outputs | 不自动接收生产账户/客户数据；仅数据策略放行后可受控兜底 |
| 国外复杂质量基准 | `gpt-4o-2024-08-06` | 对比 Plus/V3 的复杂歧义与 Structured Outputs | Structured Outputs 仍可能语义错误或拒答，且官方说明不兼容并行 Function Calling；不能替代规则终判 |

默认线上链：`qwen-turbo-2024-09-19 →（字段完整但语义仍歧义）qwen-plus-2024-12-20 →（仍不确定）澄清或转人工`。DeepSeek 与 GPT 默认只进入脱敏专用评测；国外模型受控兜底还需数据脱敏、地域/供应商策略与审批同时通过。

Router 专用评测集至少覆盖：意图准确率、必填字段抽取准确率、多意图召回、高风险请求召回、JSON 合法率、Schema 字段完整率、非法枚举率、平均/P95 延迟、每千次路由综合成本、升级调用比例、空响应与超时率。最终上线以同版本 Prompt/Schema 的盲测决定。

#### 4.1.5 Prompt 正文

```text
你是 AdOps Copilot 的“意图识别与候选字段提取模型”。你的唯一职责是输出可校验的候选结构；你不是权限系统、业务诊断 Agent 或工具执行器。
...
可做：生成候选意图与 pending_intents；仅从 current_user_query 提取新的 slot_candidates；识别它与 page_context、confirmed_conversation_slots 的冲突；报告缺失字段、风险、候选 tool_intents 和澄清候选。

硬性约束：
- 不得判断最终权限、决定 route_final、调用工具、计算指标、判断根因、执行写操作或生成业务答案。
- 不得覆盖 confirmed slots，或把任何新 candidate 标记为 confirmed。
- 不得计算相对时间的最终 UTC，不得改写 ID、数字、币种、时区、平台、MMP 或事件。
- page_context 与历史槽位只能用于比较，不能伪装为当前用户输入。
- slot_candidates 每项包含 field、raw_value、value_candidate、source、evidence_quote、observed_at、turn_id、schema_version、status；source 只能为 user_query，status 只能为 candidate。
- evidence_quote 必须是 current_user_query 中可定位的连续原文；相对时间的 value_candidate 只描述 date_expression。
- slot_conflict_candidates 明确 schema_version、existing_value/existing_source、new_value_candidate/new_source、evidence_quote、observed_at 与 turn_id；clarification_candidates.reason 只能为 missing|ambiguous|conflict。
...
Few-shot 1｜正常归因字段提取
输入摘要：current_user_query="帮我核对 Campaign cmp_102 昨天的 AppsFlyer purchase 归因差异"，page_context=null，confirmed slots=[]。
关键输出：intent_candidate=attribution_discrepancy_check；slot_candidates 含 campaign_id/cmp_102、mmp/AppsFlyer、event/purchase、time_range/昨天，均 source=user_query、status=candidate 并保留 evidence_quote；相对时间不输出 UTC。

Few-shot 2｜页面 Campaign 与用户输入冲突
输入摘要：current_user_query="看一下 cmp_202 昨天为什么掉量"，page_context.fields 中 campaign_id=cmp_101。
关键输出：slot_candidates 含 cmp_202；slot_conflict_candidates 含 existing_value=cmp_101、existing_source=page_context、new_value_candidate=cmp_202、new_source=user_query；classification_status=ambiguous；澄清候选 reason=conflict。不得覆盖页面值。

Few-shot 3｜诊断中夹带写操作
输入摘要：current_user_query="诊断 cmp_303 转化下滑，确认有问题就直接暂停投放"。
关键输出：pending_intents 保留诊断；risk_signals 含 operation_change_request；route_to=master_agent；requires_human_review_candidate=true；不得生成暂停动作或工具调用。

不要在 JSON 外输出任何文本。tool_intents 不是授权；模型报告的缺字段、冲突与路由都不是最终判定。
```

#### 4.1.6 输出 Schema 与规则归一

字段形状与语义均发生不兼容变化，因此发布 `route_model_input_v3 + route_candidate_v3`，不静默改变 v2；历史 v2 仅用于 Trace 回放，生产链只写 v3。输入 Schema 固定 Prompt 可见的七项变量，权限快照留在规则层；`route_final_v2` 的确定性语义未变，继续沿用：

```json
{
  "schema_version": "route_candidate_v3",
  "normalized_query": "核对 Campaign cmp_102 昨天的 AppsFlyer purchase 归因差异",
  "language": "zh",
  "intent_candidate": "attribution_discrepancy_check",
  "classification_status": "accepted",
  "classifier_version": "router_prompt@3.0.0",
  "calibration_version": "router_calibration@3.0.0",
  "pending_intents": [],
  "slot_candidates": [
    {"schema_version": "slot_candidate_v1", "field": "campaign_id", "raw_value": "cmp_102", "value_candidate": "cmp_102", "source": "user_query", "evidence_quote": "Campaign cmp_102", "status": "candidate", "observed_at": "2025-01-11T01:00:00Z", "turn_id": "turn_001"},
    {"schema_version": "slot_candidate_v1", "field": "time_range", "raw_value": "昨天", "value_candidate": {"date_expression": "previous_day"}, "source": "user_query", "evidence_quote": "昨天", "status": "candidate", "observed_at": "2025-01-11T01:00:00Z", "turn_id": "turn_001"}
  ],
  "slot_conflict_candidates": [],
  "missing_fields_model_reported": [],
  "risk_signals": [],
  "route_to": "attribution_discrepancy_agent",
  "tool_intents": ["get_platform_report", "get_mmp_report"],
  "clarification_candidates": [],
  "requires_human_review_candidate": false,
  "badcase_signals": []
}
...
上例中的“昨天”仍只是模型候选；Date Resolver 才能结合请求时区产出最终半开 UTC 时间窗。随后规则层可生成：
...
规则层对每次候选路由都生成 `route_final_v2`，保证后续 Guard 有唯一签名输入。`slot_candidates.status` 恒为 candidate；confirmed slots 只能来自状态服务；冲突可表达 `user_query ↔ page_context` 或 `user_query ↔ confirmed_conversation_slot`。`missing_fields_model_reported` 只是模型报告，`required_fields_final` 由 Registry 复算；`tool_intents` 只是候选，`allowed_tools` 取候选、意图、权限和阶段白名单交集；`clarification_candidates` 只供规则层选出一条最小充分问题。v3 不再携带 `retrieval_plan`，知识 Query Rewrite 由 4.4 在路由放行后执行。`ambiguous/rejected` 均由 `master_agent` 处置并令工具预算为 0；`permission_status != allowed` 时工具为空；`requires_human_review=true` 的最终交付不得为 `ready`。
...
| 指标 | 初始门槛 | 样本 |
| --- | --- | --- |
| 意图准确率/多意图召回 | >= 90% / >= 95% | 路由 Golden Set |
| 必填字段抽取与冲突召回率 | >= 95% | 缺参/冲突集 |
| 非法工具计划率 | 0 | 工具压力集 |
| 越权前置阻断率 | 100% | 权限压力集 |
| 路由 P95 | <= 2 秒 | Trace |
| JSON 合法率/Schema 完整率 | >= 99.5% | 全部路由样本 |
| 空响应与超时率 | <= 0.5% | 各模型 profile |
| 升级模型比例/每千次成本 | 记录并受预算门槛约束 | 线上 Trace |
...
| 验证 | `verification_events` | 人工/规则验证服务 | 否 | 只有有效签名事件才允许模型只读复制 confirmed cause |
...
1. **任务准入与证据计划：** 输入 `route_final`、槽位与诊断策略；确定性校验意图、权限、必填字段、允许工具和调用预算，生成证据计划。失败即澄清/阻断，不进入诊断。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**
2. **只读取数与基线可比：** 输入证据计划；Workflow Owner 调用 `get_platform_report`，下游事件按需调用 `get_mmp_report`，保留独立快照；规则检查周期、星期结构、时区、币种、成熟度、Campaign 状态与重大配置变化。核心来源失败则 `tool_degraded`，不可比则 `partial_evidence`。**LLM：否；RAG：否；只读工具：是；确定性规则：是。**
3. **指标复算与 Driver/维度贡献：** 输入工具快照；指标语义层复算公式、质量标记、Driver Tree 和可加总原子贡献。零/负值用逐项替换 waterfall；重叠维度不相加。产出 `metric_analysis` 与 Evidence；不可计算则停止对应结论。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**
4. **权威知识与已审核案例检索：** 输入异常指标、平台/地域/版本和诊断策略；调用 `search_knowledge_base` 与按需 `search_similar_cases`，执行 ACL、有效期和审核 Gate。产出受控 `retrieved_context`；案例只作检查方向。**LLM：否；RAG：是；只读工具：是；确定性规则：负责过滤。**
5. **证据约束诊断生成：** 输入 `metric_analysis`、Evidence、有效知识与 reviewed cases；模型按 `observed_fact → derived_fact → candidate_cause → pending_check` 生成草稿，每个候选原因给证据、反证/缺口、验证动作和 Owner。产出 `performance_diagnosis_draft_v2`；不得算数、确认因果或扩展工具。**LLM：是；RAG：仅消费节点 4 结果；只读工具：否；确定性规则：Schema 校验。**
6. **人工门禁与守卫移交：** 输入 Agent 草稿；规则对写操作、责任、客户影响、证据缺口和 Verification Event 做终判。可交付内容移交 4.5；否则降级、阻断或转人工。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**

#### 4.2.4 AI 模型选型与理由

AI 任务只有“把多项已验证指标、贡献、知识和证据组织为受约束诊断草稿”。历史价格口径沿用 4.1.4 的 2024 年末/2025 年初官方快照。

| 定位 | 具体模型 | 使用与理由 | 升级/停止条件 |
| --- | --- | --- | --- |
| 国内线上主模型 | `qwen-plus-2024-12-20` | 多证据、强限制、较长结构化输出；中文广告术语适配，Function Calling 可用，但草稿仍做 Schema 校验 | 默认一次生成；不让 Turbo 为省成本承担复杂归因解释 |
| 国内轻量档 | `qwen-turbo-2024-09-19` | 仅证据少、无候选原因、只需整理 observed/derived fact 的低复杂草稿 | 出现多源冲突、多个候选原因或中英文术语歧义即切 Plus |
| 国内备选/A-B | `DeepSeek-V3` / `deepseek-chat` | 供应商备选与脱敏诊断集离线 A/B | 空响应、结构漂移需重试/Schema Gate；不串行调用 |
| 国外基准/受控兜底 | `gpt-4o-mini-2024-07-18`（常规）与 `gpt-4o-2024-08-06`（复杂） | 脱敏比较数值忠实、证据绑定和因果边界 | 不自动接收生产账户数据；数据策略放行才可兜底 |

缺工具结果、不可比、证据不足、权限不足、写操作、责任/客户承诺不升级模型，直接降级、阻断或转人工。专用 Diagnosis Set 比较数值忠实率、Schema 合法率、主结论证据绑定、因果过度断言、验证动作完整率、P95、空响应率与单次综合成本。
...
6. 历史案例只能进入 case_based_check_hints，不能单独成为经验证的 supports link。
...
正常示例：metric_analysis 已确认 CPA 上升、CPC 贡献为主且币种/时区一致；输出 observed/derived facts，并把“竞价环境变化”写为 candidate_cause，绑定证据、反证缺口和人工验证动作，不写“已确认根因”。
边界示例：平台快照缺失或 current/baseline 币种不同；输出 status=partial_evidence、limitations 与补数动作，不生成候选原因，不尝试自行换算或升级模型补事实。
```
...
| 验证 | `verification_events` | 人工/规则验证服务 | 否 | 已签验证事件；不存在时不得输出 confirmed cause |
| 配置 | `checklist_policy` | 工作流配置 | 是 | 核查项、状态、停止和人工规则 |
| 输出 | `attribution_diagnosis_draft_v2` | 本 Agent | 是 | 核查草稿 |

#### 4.3.3 详细 Pipeline

1. **任务准入与对账计划：** 输入 `route_final`、槽位和 checklist policy；规则校验意图、对象、事件、双源、时间、时区、MMP、权限、允许工具与预算。产出固定九项对账计划；失败则澄清/阻断。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**
2. **双源快照与 Postback 补证：** 输入对账计划；Workflow Owner 调用 `get_platform_report`、`get_mmp_report`，按需调用 `get_postback_summary`，各自保留口径、时间、刷新与版本。双源不全则 `partial_evidence`。**LLM：否；RAG：否；只读工具：是；确定性规则：负责预算/错误处理。**
3. **可比性与差异复算：** 输入独立快照；规则核对对象、时间、时区、事件、窗口和定义版本，仅在 comparable=true 时计算 `absolute_gap`、`symmetric_gap_rate` 与显式分母的 directional rate。不可比即停止强差异结论。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**
4. **九项固定核查与权威规则检索：** 输入对账结果；配置 DAG 逐项产状态/Evidence，并调用 `search_knowledge_base` 检索窗口、映射、渠道和隐私规则。未知不得当 matched，案例不能替代当前证据。**LLM：否；RAG：是；只读工具：是；确定性规则：负责九项覆盖与过滤。**
5. **证据约束解释生成：** 输入原值、确定性差异、九项状态和有效知识；模型组织 observed/derived/candidate/pending claims，不改变分母、不归责、不确认因果。产出 `attribution_diagnosis_draft_v2`。**LLM：是；RAG：仅消费节点 4 结果；只读工具：否；确定性规则：Schema 校验。**
6. **责任与冲突门禁及守卫移交：** 输入草稿；规则对结算、责任、赔偿、欺诈、敏感日志、来源冲突和 Verification Event 终判；放行到 4.5，或转人工/降级。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**

#### 4.3.4 AI 模型选型与理由

AI 任务只有“解释已校验的双源差异与九项核查状态”。非 AI 的映射、公式、DAG 与权限留在 Pipeline/公共合同。

| 定位 | 具体模型 | 使用与理由 | 升级/停止条件 |
| --- | --- | --- | --- |
| 国内线上主模型 | `qwen-plus-2024-12-20` | 适合多源口径、九项状态和复杂中英文归因术语的结构化解释 | 默认一次生成；输出必须过同一 Schema 与因果/责任 Gate |
| 国内轻量档 | `qwen-turbo-2024-09-19` | 仅 comparable=true、无冲突且只需复述原值/差异的低复杂场景 | 出现窗口/映射/隐私归因歧义或多项 likely_issue 切 Plus |
| 国内备选/A-B | `DeepSeek-V3` / `deepseek-chat` | 国内供应商备选，脱敏 Attribution Set A/B | 不串行；空响应或结构不稳走重试/校验 |
| 国外基准/受控兜底 | `gpt-4o-mini-2024-07-18`、`gpt-4o-2024-08-06` | 分别对标常规与复杂口径解释、Structured Outputs | 生产数据不自动出境；满足数据策略才受控兜底 |

缺任一核心来源、不可比、权限不足、原始日志请求、责任/结算/赔偿或证据冲突不升级模型，直接停止、阻断或转人工。专用 Attribution Set 比较九项覆盖、差异/数值忠实、不可比阻断、证据绑定、责任过度断言、JSON/Schema、P95、空响应和成本。
...
正常示例：双源对象、事件、窗口、时区均对齐，规则层给出 symmetric_gap_rate 与九项状态；只复述确定性差异，并将 postback 延迟写为有证据的 candidate_cause，附验证动作。
边界示例：平台使用 UTC、MMP 使用 Asia/Shanghai 且窗口未对齐；输出 status=not_comparable、blocking_mismatches 与对齐动作，不自行换时区、不输出“真实差异率”或责任结论。
```
...
| 上下文 | `confirmed_conversation_slots` | 会话状态服务 | 是 | 只读确认字段，用于 Query Rewrite 保真 |
| 范围 | `allowed_knowledge_scopes` | 权限/策略服务 | 是 | 已求交集的候选检索范围，不授予模型扩展权限 |
| Schema | `retrieval_plan_schema` | 配置中心 | 是 | `retrieval_plan_candidate_v1` 的严格输出合同 |
| 安全 | `auth_context` | IAM | 是 | 检索前过滤；不传完整敏感字段给模型 |
| 检索 | `retrieved_chunks` | 权威知识检索 | 是 | 片段、版本、ACL、Owner、有效期 |
| 证据 | `knowledge_evidence_objects` | 检索证据封装器 | 是 | 每个放行 chunk 映射为 `source_type=knowledge` 的 Evidence Object，供 Claim/Judge 引用 `evidence_id` |
...
1. **范围判断与受控 Query Rewrite：** 输入 `route_final`、原问题、confirmed slots、知识 scope 与 `retrieval_plan_candidate_v1` Schema；只有 `knowledge_lookup` 继续，Query Rewrite 模型生成主题、原词、语义扩展和 metadata 过滤候选，不添加客户/账户事实。产出候选计划；具体账户诊断立即 `reroute_required`。**LLM：是；RAG：否；只读工具：否；确定性规则：负责范围和 Schema。**
2. **授权召回、重排与证据封装：** 输入检索计划；先按 tenant/role/document ACL、审核、有效期、locale 过滤，再由 `search_knowledge_base`/`search_similar_cases` 执行 BM25 + Embedding 混合召回，托管 Reranker 重排；权威库和案例库分开。每个放行 chunk 由确定性封装器生成 knowledge Evidence Object，并保留 `citation_id ↔ evidence_id` 映射。**LLM：否；RAG：是；只读工具：是；确定性规则：过滤、去重与证据封装。**
3. **Grounded 回答生成：** 输入已通过预检的片段、knowledge Evidence Objects、引用映射、适用范围和知识策略；回答模型只生成逐条引用的定义/规则/SOP 草稿，Claim 用 `proposed_evidence_ids`，展示引用用 `citation_id`，案例仅为 context-only 提示。产出 `knowledge_answer_draft_v2`。**LLM：是；RAG：仅消费受控 context；只读工具：否；确定性规则：输入预检与 Schema。**
4. **引用支持与冲突 Gate：** 输入草稿 claims、引用片段与元数据；Groundedness Judge 只给“支持/反驳/不支持”候选，规则再验证权限、版本、有效期、Owner、引用完整性和权威冲突，生成最终 Claim-Evidence Link。无可靠引用或冲突即停止/删减，不由 Judge 终判。**LLM：是（Judge）；RAG：消费既有片段；只读工具：否；确定性规则：最终 Gate。**
5. **无答案、重路由与守卫移交：** 输入通过 Gate 的草稿或停止原因；规则映射 `no_reliable_citation/citation_conflict/reroute_required/human_review_required`，记录 query、召回、重排、模型和知识版本后移交 4.5。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**

#### 4.4.4 AI 模型选型与理由

AI 模型按五个真实任务拆开，不用一个生成模型包办 RAG。国内托管 API 为线上主方案，不包含私有化/GPU/量化部署。

| AI 环节 | 国内线上主模型 | 国内升级/备选 | 国外基准或受控兜底 | 选择、成本与切换条件 |
| --- | --- | --- | --- | --- |
| Query Rewrite | `qwen-turbo-2024-09-19` | 歧义且字段完整时 `qwen-plus-2024-12-20`；`DeepSeek-V3/deepseek-chat` 离线 A/B | `gpt-4o-mini-2024-07-18` | 高频短输出优先 Turbo；缺 scope/权限/具体账户冲突直接重路由，不升级 |
| Embedding | 阿里云百炼 `text-embedding-v3` | 无在线串行备选，先回退 BM25 | 脱敏检索集可离线对比第三方 API，但 V1 不纳入生产链 | [官方发布记录](https://help.aliyun.com/zh/model-studio/newly-released-models)为 2024-07-10；[官方规格页](https://help.aliyun.com/zh/model-studio/embedding)确认 50+ 语言、8192 Token 与可选维度。其滚动页当前显示约 `¥0.5/百万输入 Token`，但不能证明 2024 年末历史价；不晚于 2025-01 的控制台账单/报价是 Phase 0 采购阻断项，补齐前不得把该数字用于项目成本结论 |
| Reranker | 阿里云百炼托管 `gte-rerank` | 无证据时跳过重排并降级，不换后来发布模型 | 在脱敏检索集上以人工相关性作为最终基准 | 阿里云官方开发者社区 [2024 年 12 月月刊](https://developer.aliyun.com/article/1647733)回顾其于 2024 年 12 月上线，[官方下线清单](https://help.aliyun.com/zh/model-studio/model-depreciation)继续确认该 legacy 模型；精确首发日、2024 历史计费方式与单价均未在滚动页保留，[官方 API](https://help.aliyun.com/zh/model-studio/text-rerank-api)可确认 usage 返回 `total_tokens`。不晚于 2025-01 的控制台报价是 Phase 0 采购阻断项，补齐前 Rerank 成本不得进入当前成本结论 |
| Grounded 回答生成 | `qwen-plus-2024-12-20` | 简单定义可用 Turbo；`DeepSeek-V3` 作为国内备选 | `gpt-4o-mini-2024-07-18` 常规、`gpt-4o-2024-08-06` 复杂基准 | 多引用与适用范围用 Plus；无可靠引用/冲突不升级，直接拒答/转 Owner |
| Groundedness Judge | `qwen-plus-2024-12-20`（仅候选判定） | `DeepSeek-V3` 离线交叉评测 | `gpt-4o-2024-08-06` 脱敏基准 | Judge 不产生最终 validated link；与人工分歧或高风险漏判时停用模型 Judge，回人工/规则 |

任何无法用项目时间窗口内官方证据确认的后来 Embedding/Reranker 都不进入清单。检索专用集分别评测 Query Rewrite 保真、Recall@5/MRR、rerank nDCG、引用准确率、Groundedness、无答案拒答、ACL 泄露、平均/P95、空响应和每千 query 的生成/Embedding/Rerank 分项成本。

#### 4.4.5 Prompt 正文

**Prompt A：受控 Query Rewrite**

```text
你是知识检索 Query Rewrite 模型。唯一职责是把已放行的 knowledge_lookup 转为 retrieval_plan_candidate_v1，不回答问题、不决定权限、不调用工具。

输入：user_query={{user_query}}；confirmed_conversation_slots={{confirmed_conversation_slots}}；knowledge_scopes={{allowed_knowledge_scopes}}；retrieval_plan_schema={{retrieval_plan_schema}}。
规则：保留原词和 ID；只添加可追溯的术语同义表达；不得添加客户/账户/配置事实；相对日期不自行换算；scope 只取输入白名单。具体账户诊断输出 reroute_required_candidate=true。
输出 JSON：schema_version=`retrieval_plan_candidate_v1`、original_query、normalized_topic、exact_terms、semantic_expansions、knowledge_scopes、locale、effective_at_candidate、must_include_metadata、forbidden_scopes、reroute_required_candidate。不要输出自然语言尾注。
正常示例：“7 天点击归因窗口是什么意思”→ topic=click_attribution_window，保留“7 天”，扩展 click-through attribution window。
边界示例：“为什么 cmp_88 昨天安装少”→ reroute_required_candidate=true，不生成账户检索范围。
```

**Prompt B：Grounded 回答生成**

```text
你是 AdOps Copilot 的 Grounded 回答生成模型。你只能使用系统已放行的引用片段和已审核案例回答。
...
3. 只用有效片段回答；每个事实性 Claim 的 proposed_evidence_ids 引用 knowledge Evidence Object 的 evidence_id，展示 citations 再使用对应 citation_id。
...
正常示例：两个同版本权威片段共同支持“7 天点击窗口”的定义；Claim 绑定 proposed_evidence_ids，citations 绑定对应 citation_id，并说明平台/版本/生效时间。
边界示例：只有 reviewed case、没有权威规则；status=no_reliable_citation，案例仅进入 case_based_check_hints，不生成规则型 claim。
```

**Prompt C：Groundedness 候选 Judge**

```text
你是 Claim-Citation 支持关系评测模型。唯一职责是判断给定片段是否支持、反驳或不支持给定 claim；你不能批准交付、改变权限、补写事实或生成答案。

输入：claims={{claims}}；citation_chunks={{citation_chunks}}；knowledge_evidence_objects={{knowledge_evidence_objects}}；knowledge_policy={{knowledge_policy}}。
只能使用片段原文与元数据。过期、越权、未审核、适用范围不符或仅案例类证据不得判为 supports。为每对关系输出可定位的 evidence_quote；证据不足输出 not_supported，不猜测。
输出严格符合 groundedness_judgment_v1：{"schema_version":"groundedness_judgment_v1","judgments":[{"claim_id":"","evidence_id":"","candidate_relation":"supports|contradicts|not_supported","evidence_quote":"","reason_code":""}],"requires_human_review_candidate":false}。
正常示例：片段逐字定义对应窗口且版本适用→supports。
边界示例：相似案例描述曾发生过同类问题但没有规则/当前数据→not_supported；最终 ClaimEvidenceLink 仍由确定性验证器生成。
```

#### 4.4.6 输出 Schema

- Query Rewrite 输出 `retrieval_plan_candidate_v1`；`effective_at_candidate` 只能是候选，最终 `retrieval_plan_v2.effective_at` 由服务端时间解析与知识策略注入。
- Groundedness Judge 输出 `groundedness_judgment_v1`；其 `candidate_relation` 不能直接写成 `ClaimEvidenceLink.validation_status=validated`，最终 link 由规则/人工验证器建立。
- `retrieved_chunks[].citation_id` 与 `knowledge_evidence_objects[].evidence_id` 的映射由确定性检索封装器生成；模型不能虚构映射。Claim 使用 evidence ID，引用卡使用 citation ID。
...
| 风险输入 | `allowed_text_fields` | 确定性投影 | 否 | Prompt A 唯一可扫描的文本白名单 |
| 风险配置 | `risk_taxonomy` | 策略中心 | 否 | Prompt A 的风险枚举与版本；未启用模型扫描时为空 |
...
1. **结构与事实校验：** 输入 Agent draft、route_final、Evidence、Claims/Links、工具运行与工作流状态；Schema/数值/公式/引用完整性规则拒绝未知字段、漂移数值和无有效 Verification Event 的 confirmed claim。产出可继续的事实集合或阻断原因。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**
2. **证据、权限与语义风险裁决：** 输入事实集合和 delivery policy；确定性检查 Claim-Evidence、ACL、visibility、冲突、新鲜度、写操作、承诺、责任、赔偿和敏感字段；安全模型仅可报告隐含语义风险候选。产出 `policy_guard_result_v2` 或停止状态。**LLM：可选，仅风险候选；RAG：否；只读工具：否；确定性规则：拥有终判。**
3. **唯一状态映射与白名单投影：** 输入 Guard 结果；按 4.6.4 首个命中规则生成唯一 `delivery_state`、人审队列和 `allowed_payload`，隐藏不可见证据但保留审计。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**
4. **受限响应格式化：** 输入 `allowed_payload` 与展示偏好；模型只能删减、排序和改写白名单表达，保持 claim ID、数字、证据、动作、限制与状态。产出 `delivery_payload_v2` 候选。**LLM：是；RAG：否；只读工具：否；确定性规则：提供白名单。**
5. **差分回检、模板降级与审计交付：** 输入格式化前后对象；规则 diff 检查新增 claim/数值/动作和隐藏失败，异常即丢弃模型结果并使用确定性模板；写入 hash、策略/模型版本和 Badcase 后交付。**LLM：否；RAG：否；只读工具：否；确定性规则：是。**

#### 4.5.4 AI 模型选型与理由

AI 任务仅有“隐含语义风险候选识别”和“白名单格式化”；权限、证据、状态与最终放行不是 AI 任务。

| 定位 | 具体模型 | 使用与理由 | 升级/停止条件 |
| --- | --- | --- | --- |
| 国内线上主模型 | `qwen-turbo-2024-09-19` | 输入短、字段已白名单化，适合低成本格式化与风险候选识别 | 格式化默认单次；失败直接模板降级，不为流畅度升级 |
| 国内复杂措辞升级 | `qwen-plus-2024-12-20` | 仅白名单内容存在复杂中英文术语或多段冲突状态，且规则已决定不可隐藏时使用 | 任何证据/权限/状态不确定不得升级，回规则或人工 |
| 国内备选/A-B | `DeepSeek-V3` / `deepseek-chat` | 脱敏 Guard/Formatter Set 的供应商 A/B | 结构/空响应风险经 Schema 与 diff Gate；不串行调用 |
| 国外基准/受控兜底 | `gpt-4o-mini-2024-07-18`；复杂边界用 `gpt-4o-2024-08-06` | 脱敏比较风险召回、格式忠实和 Structured Outputs | 不自动接收生产数据；模板始终是首选故障兜底 |

缺证据、权限失败、来源冲突、写操作、客户承诺、责任/赔偿、敏感字段或格式化新增事实时不升级模型，直接阻断、转人工或模板降级。专用 Guard/Formatter Set 评测高风险召回、无依据 claim 阻断、delivery_state 准确率、格式化新增 claim/数值率、JSON/Schema、P95、空响应和成本。

#### 4.5.5 Prompt 正文

**Prompt A：语义风险候选识别（可选）**

```text
你是交付语义风险候选分类器。只识别文本是否含写操作、客户承诺、责任归属、合同/赔偿、敏感信息或隐藏失败的语义候选；不得决定 delivery_state、权限或最终放行。
输入：allowed_text_fields={{allowed_text_fields}}；risk_taxonomy={{risk_taxonomy}}。
输出严格 JSON：{"risk_candidates":[{"code":"","evidence_quote":"","confidence_band":"low|medium|high"}],"schema_version":"delivery_risk_candidate_v1"}。evidence_quote 必须来自输入，证据不足返回空数组。
正常示例：“建议人工核查预算配置”→不等同写操作，可返回空或低风险候选。
边界示例：“系统已替客户暂停并保证赔付”→operation_change、customer_commitment、compensation 三类候选；最终状态仍由规则层决定。
```
...
正常示例：ready 的白名单包含两个 observed facts 和一个人工核查动作；仅调整标题/顺序，所有 ID、数字和限制逐字保留。
边界示例：delivery_state=citation_conflict；首屏展示冲突和 Owner 下一步，不删失败状态，不把两个来源合并成一个确定结论。
```
...
`conversation_slots` 公共合同：服务端只持久化 `confirmed_conversation_slots` 与待处理 `slot_conflicts`；模型输出的 `slot_candidates` 先按 A.8 的 `SlotCandidate` 校验，再由 Date Resolver、枚举/ID 规则或用户确认决定是否写入 confirmed 状态。任何 Agent 不得直接把 candidate 改为 confirmed。自然语言候选保留 `raw_value/evidence_quote/observed_at/turn_id/schema_version`，页面字段保留签名 context 与 `observed_at`，confirmed 字段额外保留 `confirmed_at`。冲突未解决时不能获得业务工具预算。
...
```json
{
  ...
  "query_params_normalized": {
    ...
    "event_names": ["install"],
    "current_period": {"start_inclusive_utc": "2025-01-09T16:00:00Z", "end_exclusive_utc": "2025-01-10T16:00:00Z"},
  ...
  "observed_at": "2025-01-10T16:00:00Z",
  "retrieved_at": "2025-01-11T02:00:00Z",
  "time_window": {"start_inclusive_utc": "2025-01-09T16:00:00Z", "end_exclusive_utc": "2025-01-10T16:00:00Z"},
  "display_timezone": "Asia/Shanghai",
  "currency": "USD",
  ...
  "missing_fields": [],
  "errors": [],
  "integrity_hash": "hmac-sha256:7026bcf1ae1aabacd77d3a183d9889b9383fb3009226b7116fa2c47116f2d1c6"
}
```
...
```json
{
  ...
  "training_allowed": false,
  "limitations": [],
  "integrity_hash": "hmac-sha256:4ae592f7cf95222534228891482ffdf135776236db2795e5fd6d769af21922cd"
}
```
...
```json
{
  ...
  "verified_at": "2025-01-11T03:00:00Z",
  "verified_scope": {"tenant_id": "tenant_demo", "account_id": "A_DEMO", "app_id": "APP_DEMO", "campaign_id": "C_DEMO_123", "time_window": {"start_inclusive_utc": "2025-01-09T16:00:00Z", "end_exclusive_utc": "2025-01-10T16:00:00Z"}},
...
- Embedding/Rerank：线上 PoC 使用阿里云百炼托管 `text-embedding-v3` 与 legacy `gte-rerank`，并保留 BM25 降级；不评估私有化、自建 GPU、量化或本地部署。二者必须在 Recall@K、MRR/nDCG、引用准确率、延迟、Token 成本和供应商可用性上盲测，且在采购档案中冻结不晚于 2025-01 的官方版本/报价证据。
...
Query 改写由 4.4 在 `route_final` 放行 `knowledge_lookup` 后执行；总控只负责意图/槽位候选，不再生成检索计划。模型输出候选计划，服务端再注入最终 ACL、scope 与 effective time：

```json
{
  "schema_version": "retrieval_plan_candidate_v1",
  ...
  "locale": "zh-CN",
  "effective_at_candidate": null,
  "must_include_metadata": ["source_version", "owner", "effective_at", "expires_at"],
  "forbidden_scopes": ["other_tenant"],
  "reroute_required_candidate": false
}
```

改写不得添加用户没有提供的客户、账户、配置或结论；所有扩展词保留来源并可审计。`effective_at_candidate` 不是最终过滤时间，服务端按请求时间、时区和知识策略解析后才写入检索执行参数。
...
- Query 改写错：4.4 Query Rewrite Prompt/词典修复。
...
输入：
- case: {{golden_case}}
- route_candidate_v3: {{route_candidate_v3}}
- route_final: {{route_final}}
- intent_registry: {{intent_registry}}
...
| `confirmed_conversation_slots` / `slot_conflicts` | 已确认 campaign/event/period 及待澄清冲突；不保存自由文本总结 | 当前 trace/会话 | 多轮任务连续性与来源追溯 | 任务关闭后按策略清理 | tenant 隔离、字段最小化；candidate 不得直升 confirmed |
...
| 任务 | 线上国内 model profile | 单次预算假设 | 基准/降级 |
| --- | --- | --- | --- |
| 意图/槽位 | `qwen-turbo-2024-09-19`；完整字段复杂歧义才 `qwen-plus-2024-12-20` | 输入 2k、输出 600 tokens | 规则终判；失败澄清/人工 |
| 投放/归因解释 | `qwen-plus-2024-12-20`；低复杂复述可 Turbo | 输入 8k、输出 2k tokens | DeepSeek-V3 A/B；GPT 脱敏基准；缺证据即降级 |
| Query Rewrite/知识回答 | Turbo 改写；`text-embedding-v3`；`gte-rerank`；Plus 回答/Judge | 生成输入 6k、输出 1.5k；Embedding/Rerank 单列 | BM25、无答案拒答；GPT 脱敏基准 |
| 响应格式化 | `qwen-turbo-2024-09-19` | 输入 4k、输出 1.2k tokens | 确定性模板 |
| JudgeAI | `qwen-plus-2024-12-20` 离线批处理 | 按抽样/评测运行 | DeepSeek/GPT 脱敏对照，规则先筛 |

Prompt 不硬编码供应商品牌，但生产配置必须绑定本章已评测的具体 `model_profile` 与时间快照：线上国内主链优先 Qwen，DeepSeek-V3 为国内备选/A-B，GPT-4o mini/4o 仅作脱敏基准或数据策略放行后的受控兜底。任何主备切换必须保持同一 Prompt/Schema，回归结构化输出、中文/英文广告术语、数值忠实、证据遵循、P95、空响应、数据合规和成本；不得引入项目时间窗口之外的模型。
...
retrieval_cost = embedding_input_tokens * embedding_price_per_token
               + rerank_input_tokens * rerank_price_per_token
...
| `gold_route_002` | 页面 Campaign 冲突 | 页面 `cmp_101`、用户说 `cmp_202` | v3 产 candidate + conflict，规则层澄清且工具预算为 0 | 覆盖页面或把候选写 confirmed | 总控 |
| `gold_route_003` | 相对时间 | 用户说“昨天”且时区 Asia/Shanghai | 模型保留 raw/evidence；Date Resolver 算半开 UTC 窗口 | 模型自行决定最终 UTC | 总控 |
...
| --- | ---: | --- | --- | --- | --- |
| 平台 | 1,250 | 2025-01-04 00:00-23:59 | Asia/Shanghai | install_v3 | 否 |
| MMP | 900 | 2025-01-04 00:00-23:59 | UTC | install_v3 | 否 |
...
| `reg_007` | route candidate 直接写 confirmed slot | 总控 Prompt/会话状态/Schema | 升级 `route_candidate_v3`，candidate 与 confirmed 专用 Schema 分离 | 模型输出不得含 confirmed 状态或 confirmed_at |
| `reg_008` | Query Rewrite 仍由总控生成 | 路由/检索责任 | 从 route candidate 移除 retrieval plan，由 4.4 放行后生成 | 总控候选不得扩展检索 scope |
| `reg_009` | 高风险意图被 accepted 后授权工具 | RouteFinal Schema/范围策略 | 对写操作、客户承诺、SDK/素材深诊强制 master + blocked + 零工具 + 人审 | 高风险 intent 不得进入业务 Agent 或获得工具预算 |
...
```json
{
  ...
  "event_names": ["install"],
  "current_period": {"start_inclusive_utc": "2025-01-09T16:00:00Z", "end_exclusive_utc": "2025-01-10T16:00:00Z"},
  "baseline_period": {"start_inclusive_utc": "2025-01-02T16:00:00Z", "end_exclusive_utc": "2025-01-03T16:00:00Z"},
  "display_timezone": "Asia/Shanghai",
  "currency": "USD",
...
A.6 中两个 `integrity_hash` 可独立复算：使用非秘密演示 key `synthetic-demo-key`，对 UTF-8 编码的规范串 `tenant_demo|C_DEMO_123|install|<十进制整数字符串>|2025-01-09T16:00:00Z|2025-01-10T16:00:00Z` 计算 HMAC-SHA256 并输出小写十六进制；`<十进制整数字符串>` 分别为 `1120` 与 `1080`，不允许空值或额外空格。`resource_ids_hash` 与 `auth_snapshot_hash` 只作为合成不透明标识展示，不宣称可由本附件复算。生产环境必须使用密钥管理系统、canonical JSON 与不可变审计，不能复用演示 key。
...
| Slot status/source | `candidate`、`confirmed`、`conflict`；`user_query`、`page_context`、`confirmed_conversation_slot` | 会话状态/AI PM |
| 版本资产 | route Prompt v3、各 Agent Prompt、Judge、model profile、workflow、rule、tool schema、KB/index、delivery policy | 各资产 Owner |

上线前，注册表、Prompt 输入输出、Schema、文档术语与汇总版必须通过自动一致性扫描。

Intent Registry 的**授权执行路径**唯一 Workflow Owner 映射如下；权限/范围/字段失败或高风险阻断时统一由 `master_agent` 处置，不把被阻断任务交给业务 Owner：

| intent | 唯一 Owner |
| --- | --- |
| `campaign_performance_diagnosis` | `performance_diagnosis_agent` |
| `attribution_discrepancy_check` | `attribution_discrepancy_agent` |
| `knowledge_lookup` | `knowledge_retrieval_agent` |
| `case_escalation_summary` | `trace_summary_builder` |
| `feedback_badcase` | `badcase_intake` |
| `operation_change_request`、`customer_commitment_request`、`sdk_creative_deep_diagnosis`、`unknown` | `master_agent` |
  ...
  "$defs": {
    ...
    "Entity": {
      ...
      "properties": {
        ...
        "source": {"enum": ["user_query", "page_context", "confirmed_conversation_slot"]},
    ...
    "SlotCandidate": {
      ...
      "properties": {
        ...
        "raw_value": {"type": ["string", "number", "boolean"]},
        "value_candidate": {"type": ["string", "number", "boolean", "object", "array", "null"]},
        "source": {"const": "user_query"},
        "evidence_quote": {"type": "string", "minLength": 1},
        "status": {"const": "candidate"},
        "observed_at": {"type": "string", "format": "date-time"},
        "turn_id": {"type": "string", "minLength": 1}
      },
      "required": ["schema_version", "field", "raw_value", "value_candidate", "source", "evidence_quote", "status", "observed_at", "turn_id"],
      "allOf": [
        {"if": {"properties": {"field": {"const": "time_range"}}, "required": ["field"]}, "then": {"properties": {"value_candidate": {"type": "object", "properties": {"date_expression": {"type": "string", "minLength": 1}}, "required": ["date_expression"], "additionalProperties": false}}}}
      ],
      "additionalProperties": false
    },
    "ConfirmedConversationSlot": {
      "type": "object",
      "properties": {
        "schema_version": {"const": "confirmed_conversation_slot_v1"},
        "field": {"type": "string", "minLength": 1},
        "value": {"type": ["string", "number", "boolean", "object", "array", "null"]},
        "source": {"const": "confirmed_conversation_slot"},
        "origin_source": {"enum": ["user_query", "page_context"]},
      ...
      "required": ["schema_version", "field", "value", "source", "origin_source", "raw_value", "evidence_quote", "observed_at", "confirmed_at", "turn_id"],
      "allOf": [
        {"if": {"properties": {"origin_source": {"const": "user_query"}}, "required": ["origin_source"]}, "then": {"properties": {"raw_value": {"type": ["string", "number", "boolean"]}, "evidence_quote": {"type": "string", "minLength": 1}}}}
      ],
      "additionalProperties": false
    },
    "SlotConflictCandidate": {
      "type": "object",
      "properties": {
        "schema_version": {"const": "slot_conflict_candidate_v1"},
        "field": {"type": "string", "minLength": 1},
        "existing_value": {"type": ["string", "number", "boolean", "object", "array", "null"]},
        "existing_source": {"enum": ["page_context", "confirmed_conversation_slot"]},
        "new_value_candidate": {"type": ["string", "number", "boolean", "object", "array", "null"]},
        "new_source": {"const": "user_query"},
        "evidence_quote": {"type": "string", "minLength": 1},
        "observed_at": {"type": "string", "format": "date-time"},
        "turn_id": {"type": "string", "minLength": 1}
      },
      "required": ["schema_version", "field", "existing_value", "existing_source", "new_value_candidate", "new_source", "evidence_quote", "observed_at", "turn_id"],
      "additionalProperties": false
    },
    ...
    "RouteModelInputV3": {
      "type": "object",
      "properties": {
        "schema_version": {"const": "route_model_input_v3"},
        "current_user_query": {"type": "string", "minLength": 1},
        "page_context": {
          "type": ["object", "null"],
          "properties": {
            "schema_version": {"const": "page_context_v1"},
          ...
          "required": ["schema_version", "context_id", "observed_at", "fields"],
          "additionalProperties": false
        },
        "confirmed_conversation_slots": {"type": "array", "items": {"$ref": "#/$defs/ConfirmedConversationSlot"}},
        "recent_dialogue": {
          "type": "array",
          "maxItems": 6,
          "items": {
          ...
          "properties": {
            ...
            "entries": {
              ...
              "items": {
                "type": "object",
                "properties": {
                  "intent": {"enum": ["campaign_performance_diagnosis", "attribution_discrepancy_check", "knowledge_lookup", "case_escalation_summary", "feedback_badcase", "operation_change_request", "customer_commitment_request", "sdk_creative_deep_diagnosis", "unknown"]},
                  "required_fields": {"type": "array", "uniqueItems": true, "items": {"type": "string"}},
                  "workflow_owner": {"enum": ["performance_diagnosis_agent", "attribution_discrepancy_agent", "knowledge_retrieval_agent", "trace_summary_builder", "badcase_intake", "master_agent"]},
                  "candidate_tools": {"type": "array", "uniqueItems": true, "items": {"enum": ["get_platform_report", "get_mmp_report", "get_postback_summary", "search_knowledge_base", "search_similar_cases"]}}
                },
                "required": ["intent", "required_fields", "workflow_owner", "candidate_tools"],
                "additionalProperties": false
              }
            }
          },
          ...
          "properties": {
            ...
            "entries": {
              ...
              "items": {
                ...
                "properties": {
                  "field": {"type": "string"},
                  "value_type": {"enum": ["string", "number", "boolean", "id", "enum", "date_expression"]},
                  "allowed_values": {"type": "array", "items": {"type": ["string", "number", "boolean"]}},
                  "allowed_sources": {"type": "array", "uniqueItems": true, "items": {"enum": ["user_query", "page_context", "confirmed_conversation_slot"]}},
                  "extraction_rule": {"type": "string"}
                },
                "required": ["field", "value_type", "allowed_values", "allowed_sources", "extraction_rule"],
                "additionalProperties": false
              }
            }
          },
            ...
            "entries": {
              ...
              "items": {
                ...
                "properties": {
                  ...
                  "purpose": {"type": "string"},
                  "allowed_intents": {"type": "array", "uniqueItems": true, "items": {"enum": ["campaign_performance_diagnosis", "attribution_discrepancy_check", "knowledge_lookup"]}}
                },
                "required": ["tool_name", "purpose", "allowed_intents"],
                "additionalProperties": false
              }
            }
          },
      ...
      "required": ["schema_version", "current_user_query", "page_context", "confirmed_conversation_slots", "recent_dialogue", "intent_registry", "slot_schema", "tool_registry_summary"],
      "additionalProperties": false
    },
    "RetrievalPlanCandidate": {
      "type": "object",
      "properties": {
        "schema_version": {"const": "retrieval_plan_candidate_v1"},
        ...
        "knowledge_scopes": {"type": "array", "uniqueItems": true, "items": {"type": "string"}},
        "locale": {"type": "string"},
        "effective_at_candidate": {"type": ["string", "null"]},
        ...
        "reroute_required_candidate": {"type": "boolean"}
      },
      "required": ["schema_version", "original_query", "normalized_topic", "exact_terms", "semantic_expansions", "knowledge_scopes", "locale", "effective_at_candidate", "must_include_metadata", "forbidden_scopes", "reroute_required_candidate"],
      "allOf": [
        {"if": {"properties": {"reroute_required_candidate": {"const": false}}, "required": ["reroute_required_candidate"]}, "then": {"properties": {"knowledge_scopes": {"minItems": 1}}}}
      ],
      "additionalProperties": false
    },
    "RetrievalPlan": {
    ...
    "JudgeResult": {
      "type": "object",
      "properties": {
        "judge_version": {"enum": ["router_judge_v3", "performance_judge_v2", "attribution_judge_v2", "knowledge_judge_v2", "delivery_safety_judge_v2"]},
        "pass": {"type": "boolean"},
      ...
      "allOf": [
        ...
        {"if": {"properties": {"judge_version": {"const": "router_judge_v3"}}, "required": ["judge_version"]}, "then": {"properties": {"dimension_scores": {"$ref": "#/$defs/RouterJudgeScores"}}, "required": ["errors", "evidence", "suggested_asset_owner"]}},
    ...
    "RouteCandidateV3": {
      ...
      "required": ["schema_version", "normalized_query", "language", "intent_candidate", "classification_status", "classifier_version", "calibration_version", "pending_intents", "slot_candidates", "slot_conflict_candidates", "missing_fields_model_reported", "risk_signals", "route_to", "tool_intents", "clarification_candidates", "requires_human_review_candidate", "badcase_signals"],
      "additionalProperties": false
    },
    "RouteFinal": {
      "type": "object",
      ...
      "allOf": [
        {"if": {"properties": {"intent_final": {"const": "campaign_performance_diagnosis"}, "workflow_state": {"const": "authorized"}}, "required": ["intent_final", "workflow_state"]}, "then": {"properties": {"owner": {"const": "performance_diagnosis_agent"}, "allowed_tools": {"items": {"enum": ["get_platform_report", "get_mmp_report", "search_knowledge_base", "search_similar_cases"]}}}}},
        {"if": {"properties": {"intent_final": {"const": "attribution_discrepancy_check"}, "workflow_state": {"const": "authorized"}}, "required": ["intent_final", "workflow_state"]}, "then": {"properties": {"owner": {"const": "attribution_discrepancy_agent"}, "allowed_tools": {"items": {"enum": ["get_platform_report", "get_mmp_report", "get_postback_summary", "search_knowledge_base"]}}}}},
        {"if": {"properties": {"intent_final": {"const": "knowledge_lookup"}, "workflow_state": {"const": "authorized"}}, "required": ["intent_final", "workflow_state"]}, "then": {"properties": {"owner": {"const": "knowledge_retrieval_agent"}, "allowed_tools": {"items": {"enum": ["search_knowledge_base", "search_similar_cases"]}}, "max_tool_calls": {"maximum": 2}}}},
        {"if": {"properties": {"intent_final": {"const": "case_escalation_summary"}, "workflow_state": {"const": "authorized"}}, "required": ["intent_final", "workflow_state"]}, "then": {"properties": {"owner": {"const": "trace_summary_builder"}, "allowed_tools": {"maxItems": 0}, "max_tool_calls": {"const": 0}}}},
        {"if": {"properties": {"intent_final": {"const": "feedback_badcase"}, "workflow_state": {"const": "authorized"}}, "required": ["intent_final", "workflow_state"]}, "then": {"properties": {"owner": {"const": "badcase_intake"}, "allowed_tools": {"maxItems": 0}, "max_tool_calls": {"const": 0}}}},
        {"if": {"properties": {"intent_final": {"enum": ["operation_change_request", "customer_commitment_request", "sdk_creative_deep_diagnosis"]}}, "required": ["intent_final"]}, "then": {"properties": {"owner": {"const": "master_agent"}, "workflow_state": {"const": "blocked"}, "allowed_tools": {"maxItems": 0}, "max_tool_calls": {"const": 0}, "requires_human_review": {"const": true}}}},
        {"if": {"properties": {"intent_final": {"const": "unknown"}}, "required": ["intent_final"]}, "then": {"properties": {"owner": {"const": "master_agent"}, "workflow_state": {"enum": ["clarification_required", "blocked"]}, "allowed_tools": {"maxItems": 0}, "max_tool_calls": {"const": 0}}}},
    ...
    "GroundednessJudgment": {
      "type": "object",
      "properties": {
        "schema_version": {"const": "groundedness_judgment_v1"},
        "judgments": {
          "type": "array",
          "items": {
            "type": "object",
            "properties": {
              "claim_id": {"type": "string"},
              "evidence_id": {"type": "string"},
              "candidate_relation": {"enum": ["supports", "contradicts", "not_supported"]},
              "evidence_quote": {"type": "string"},
              "reason_code": {"type": "string"}
            },
            "required": ["claim_id", "evidence_id", "candidate_relation", "evidence_quote", "reason_code"],
            "allOf": [
              {"if": {"properties": {"candidate_relation": {"enum": ["supports", "contradicts"]}}, "required": ["candidate_relation"]}, "then": {"properties": {"evidence_quote": {"minLength": 1}}}},
              {"if": {"properties": {"candidate_relation": {"const": "not_supported"}}, "required": ["candidate_relation"]}, "then": {"properties": {"reason_code": {"minLength": 1}}}}
            ],
            "additionalProperties": false
          }
        },
      ...
      "required": ["schema_version", "judgments", "requires_human_review_candidate"],
      "additionalProperties": false
    },
    "DeliveryRiskCandidate": {
      "type": "object",
      "properties": {
        "schema_version": {"const": "delivery_risk_candidate_v1"},
        "risk_candidates": {
          "type": "array",
          "items": {
            "type": "object",
            ...
            "required": ["code", "evidence_quote", "confidence_band"],
            "additionalProperties": false
          }
        }
      },
      "required": ["schema_version", "risk_candidates"],
      "additionalProperties": false
    },
    "AgentDraft": {
      "type": "object",
      ...
      "allOf": [
        {"if": {"properties": {"schema_version": {"const": "performance_diagnosis_draft_v2"}}}, "then": {"properties": {"status": {"enum": ["answered", "partial_evidence", "tool_degraded", "need_clarification", "out_of_scope", "human_review_required"]}}, "required": ["problem_statement"]}},
        {"if": {"properties": {"schema_version": {"const": "attribution_diagnosis_draft_v2"}}}, "then": {"properties": {"status": {"enum": ["answered", "partial_evidence", "not_comparable", "citation_conflict", "human_review_required"]}, "checklist": {"minItems": 9, "maxItems": 9}}, "required": ["comparability", "checklist"]}},
        {"if": {"properties": {"schema_version": {"const": "knowledge_answer_draft_v2"}}}, "then": {"properties": {"status": {"enum": ["answered", "no_reliable_citation", "citation_conflict", "reroute_required", "human_review_required"]}}, "required": ["answer_summary", "citations", "applicability", "common_misunderstandings", "case_based_check_hints", "knowledge_gap"]}}
      ],
      "additionalProperties": false
    },
    "PolicyGuardResult": {
